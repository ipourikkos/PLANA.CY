"""PLANA.CY Direct ArcGIS Parcel Indexer V8 Online-First — 2GB crash-safe.
...
- Resolves planning coefficients from the bundled verified planning-zone registry
  using the official DLS development-plan and zone polygons.
...
def load_verified_zone_registry() -> tuple[
    dict[tuple[str, str], dict[str, float | None]],
    dict[str, dict[str, float | None]],
]:
    """Load authoritative plan+zone coefficients from the bundled registry.

    The previous indexer inferred official parameters from medians of existing
    parcel rows. That can perpetuate missing or incorrect values. V8 now uses
    the verified planning-zone registry generated from the official zone legend.
    A zone-only consensus is used only when every plan record agrees.
    """
    try:
        payload = json.loads(ZONE_REGISTRY_SNAPSHOT.read_text(encoding="utf-8"))
        rows = payload.get("records") if isinstance(payload, dict) else payload
    except Exception as exc:
        print(f"verified zone registry unavailable: {exc!r}", flush=True)
        return {}, {}

    exact: dict[tuple[str, str], dict[str, float | None]] = {}
    grouped: dict[str, list[dict[str, float | None]]] = defaultdict(list)
    for row in rows or []:
        if not isinstance(row, dict):
            continue
        plan_key = _registry_key(row.get("development_plan"))
        zone_key = _registry_key(row.get("zone_code"))
        if not zone_key:
            continue
        coeff = {
        ...
        if plan_key:
            exact[(plan_key, zone_key)] = coeff
        grouped[zone_key].append(coeff)

    consensus: dict[str, dict[str, float | None]] = {}
    for zone_key, candidates in grouped.items():
        signatures = {
            tuple(candidate.get(field) for field in ("density_percent", "coverage_percent", "max_floors", "max_height_m"))
            for candidate in candidates
        }
        if len(signatures) == 1:
            consensus[zone_key] = candidates[0]
    print(
        f"loaded verified zone registry exact={len(exact)} consensus={len(consensus)}",
        flush=True,
    )
    return exact, consensus


class DevelopmentPlanIndex:
    """Spatial index for official DLS development-plan polygons."""
    ...
    def __init__(self, features: list[dict[str, Any]]):
        ...
        for feature in features:
            ...
            props = property_dict(feature)
            name = clean_text(props.get("DEVP_NM_G"))
        ...
        for candidate in self.tree.query(parcel_geom):
            try:
                ...
                try:
                    ...
                    continue
            if not plan_geom.intersects(parcel_geom):
                continue
            try:
                area = float(plan_geom.intersection(parcel_geom).area)
            except Exception:
                area = 0.0
            if area > best_area:
                best_area = area
                best_name = self.names[index]
        return best_name
...
def parcel_row(
    ...
    *,
    ...
    plan_index: DevelopmentPlanIndex,
    zone_registry_exact: dict[tuple[str, str], dict[str, float | None]],
    zone_registry_consensus: dict[str, dict[str, float | None]],
    ...
    development_plan = plan_index.plan_for(geom_json)
    plan_key = _registry_key(development_plan)
    raw_zones = zone_index.zones_for(geom_json, area)
    zones: list[dict[str, Any]] = []
    resolved_coefficients: dict[str, dict[str, float | None]] = {}
    exact_matches = 0
    consensus_matches = 0
    for zone in raw_zones:
        zone_name = str(zone.get("zone") or "")
        zone_key = _registry_key(zone_name)
        coeff = zone_registry_exact.get((plan_key, zone_key)) if plan_key else None
        source = "verified_plan_zone"
        if not coeff:
            coeff = zone_registry_consensus.get(zone_key)
            source = "verified_zone_consensus"
        coeff = coeff or {}
        if coeff:
            if source == "verified_plan_zone":
                exact_matches += 1
            else:
                consensus_matches += 1
            resolved_coefficients[zone_name] = coeff
        zones.append({
            **zone,
            ...
            "coefficient_source": source if coeff else "missing",
        })
    coeff, coeff_status = coefficient_for_zones(zones, resolved_coefficients)
    ...
    return {
        ...
        **score,
        ...
        "raw_summary": {
            ...
            "coefficient_source": (
                "verified_plan_zone_registry" if exact_matches else
                "verified_zone_consensus" if consensus_matches else "pending"
            ),
...
async def run_cycle(sb: Any, client: httpx.AsyncClient, run_id: str, cycle_seconds: int) -> None:
    ...
    print("PLANA Direct ArcGIS Index V8 VERIFIED-ZONE 2GB RESUMABLE starting", flush=True)
    print(
    ...
    district_features, municipality_features, quarter_features, plan_features, zone_features = await asyncio.gather(
        ...
        fetch_all_features(client, DEVELOPMENT_PLAN_QUERY, out_fields="DEVP_NM_G,OBJECTID", return_geometry=True),
    ...
    plan_index = DevelopmentPlanIndex(plan_features)
    zone_index = ZoneIndex(zone_features)

    del district_features, municipality_features, quarter_features, plan_features, zone_features
    gc.collect()

    zone_registry_exact, zone_registry_consensus = await asyncio.to_thread(load_verified_zone_registry)
    rss = current_rss_mb()
    ready = (
        f"reference indexes ready districts={len(district_lookup)} municipalities={len(municipality_lookup)} "
        f"quarters={len(quarter_lookup)} plans={len(plan_index.geometries)} zones={len(zone_index.geometries)} "
        f"verified_exact={len(zone_registry_exact)} verified_consensus={len(zone_registry_consensus)}"
    )
        ...
        for feature in work:
            ...
            try:
                row = parcel_row(
                    ...
                    zone_registry_exact=zone_registry_exact,
                    zone_registry_consensus=zone_registry_consensus,
...
async def main() -> None:
    ...
    claim_retry_seconds = max(5, env_int("PLANA_INDEX_LEASE_CLAIM_RETRY_SECONDS", 15))
    while True:
        claimed = await claim_worker(sb, INDEX_WORKER_LEASE, run_id, lease_seconds=lease_seconds)
        if claimed:
            break
        print(
            "PLANA INDEX lease is currently owned by another instance; "
    ...
    print(f"PLANA INDEX lease acquired run_id={run_id}", flush=True)
