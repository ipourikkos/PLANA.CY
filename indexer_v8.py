...
def load_zone_coefficients(sb: Any) -> dict[str, dict[str, float | None]]:
    ...
    try:
        ...
        # Zone coefficients are optional enrichment. Never block the parcel crawl
        # by scanning the full inherited parcel table when the RPC times out.
        print(
            f"zone coefficient RPC unavailable; continuing without learned coefficients: {exc!r}",
            flush=True,
        )
        return {}
