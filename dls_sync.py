...
CONCURRENCY=max(1,min(int(os.getenv("DLS_SYNC_CONCURRENCY","2")),4))
BATCH=max(25,min(int(os.getenv("DLS_SYNC_BATCH","100")),200))
UPSERT_BATCH=100
...
async def get_json(client,url,params,retries=6,post=False):
    delay=1.0
    for attempt in range(retries):
        try:
            r=await (client.post(url,data=params) if post else client.get(url,params=params))
            r.raise_for_status()
            data=r.json()
            ...
            if e.response.status_code not in (408,429,500,502,503,504) or attempt==retries-1:
                raise
            await asyncio.sleep(delay); delay=min(delay*2,20)
        ...
        async def fetch(chunk):
            async with sem:
                return await get_json(client,q,{"f":"geojson","objectIds":",".join(map(str,chunk)),
                  "outFields":"*","returnGeometry":"true","outSR":"4326"},post=True)
