...
def build_decision_intelligence(
    ...
    *,
    ...
    for option in proposals.get("development_options") or []:
        ...
        planning_score = 82 if status == "allowed" else 58
        density_efficiency = _clamp((gross / parcel_area * 55.0) if parcel_area else 35.0, 25, 95)
        market_score = 50.0
        financial_score = 45.0
        parking_score = 65.0
        if kind == "apartments":
            market_score = _clamp(38 + min(obs_count, 40) * 1.0 + min(source_count, 4) * 4)
            financial_score = 70 if sale_mid else 45
            ...
            if parking_low and parking_capacity_high:
                parking_score = _clamp(100 * parking_capacity_high / parking_low, 25, 95)
        elif kind == "house":
            market_score = 45 if not sale_mid else 58
            financial_score = 48
        elif kind == "mixed_use":
            market_score = 42
            financial_score = 48
    ...
    label = "Strong opportunity" if score >= 78 else ("Promising opportunity" if score >= 65 else ("Review opportunity" if score >= 50 else "High-uncertainty opportunity"))
    return {
...
OPPORTUNITY_SCAN_LIMIT = int(os.getenv("PLANA_OPPORTUNITY_SCAN_LIMIT", "60"))
OPPORTUNITY_DETAIL_LIMIT = int(os.getenv("PLANA_OPPORTUNITY_DETAIL_LIMIT", "20"))
...
class OpportunityFinderRequest(BaseModel):
    ...
    max_results: int = Field(default=12, ge=3, le=25)
...
async def _query_parcels_in_bbox(payload: OpportunityFinderRequest) -> list[dict[str, Any]]:
    params = {
        ...
        "where": "1=1",
        "geometry": json.dumps({
            "xmin": payload.west, "ymin": payload.south,
            "xmax": payload.east, "ymax": payload.north,
            "spatialReference": {"wkid": 4326},
        }),
        ...
        "returnGeometry": "true",
        "resultRecordCount": OPPORTUNITY_SCAN_LIMIT,
    }
    client = state.get("http")
    if not client:
        raise HTTPException(status_code=503, detail="HTTP service is not ready yet.")
    response = await client.get(PARCEL_QUERY, params=params, timeout=20.0)
    if response.status_code != 200:
        raise HTTPException(status_code=502, detail="DLS opportunity scan failed.")
    data = response.json()
    if data.get("error"):
        raise HTTPException(status_code=502, detail=f"DLS opportunity scan error: {data['error']}")
...
async def _finder_candidate(feature: dict[str, Any], payload: OpportunityFinderRequest, semaphore: asyncio.Semaphore) -> dict[str, Any] | None:
    ...
    components = {
        "financial": float(option.get("financial_score") or 45),
        "market": float(option.get("market_score") or 45),
        "development": (float(option.get("development_efficiency_score") or 45) + scale_fit) / 2,
        "planning": float(option.get("planning_score") or 45),
        "site": float(option.get("site_constraint_score") or 45),
    }
    personalised = round(sum(components[k] * weights[k] for k in weights))
    ...
    if "underused" in payload.priorities and underused_percent is not None:
        personalised = round(.65 * personalised + .35 * underused_percent)
    ...
    return {
        ...
        "score_label": "Standout" if personalised >= 82 else ("Strong" if personalised >= 72 else "Worth a look"),
...
async def find_opportunities(payload: OpportunityFinderRequest) -> dict[str, Any]:
    ...
    # Prioritise parcels whose map geometry roughly fits the selected development scale.
    # This avoids spending a GeneralParcelIdentify request on every parcel in the envelope.
    ...
    selected = sorted(features, key=geometry_rank)[:OPPORTUNITY_DETAIL_LIMIT]
    semaphore = asyncio.Semaphore(10)
    rows = await asyncio.gather(*[_finder_candidate(f, payload, semaphore) for f in selected])
    candidates = [x for x in rows if x]
    ...
    result = {
        ...
        "note": "Opportunity ranking is a preliminary DLS-based screening. Market and acquisition budget are verified when a parcel is opened.",
    }
...
let dlsOverlay=null;
if(L.esri&&L.esri.dynamicMapLayer)dlsOverlay=L.esri.dynamicMapLayer({url:"https://eservices.dls.moi.gov.cy/arcgis/rest/services/National/CadastralMap_EN/MapServer",opacity:.72,layers:[0],updateInterval:250});
...
async function post(url,body,signal=null){const r=await fetch(url,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(body),signal});let d={};try{d=await r.json()}catch{}if(!r.ok)throw new Error(d.detail||"Request failed");return d}
...
function renderDecision(){
 ...
 $("scoreSection").classList.remove("hidden");const c=d.score_components||{};$("scoreBody").innerHTML=`<div class="score-row"><div><div class="eyebrow" style="color:rgba(255,255,255,.65)">PLANA SCORE</div><div class="score-num">${esc(d.plana_score)}<small>/100</small></div></div><div><div class="score-label">${esc(d.score_label)}</div><div class="score-sub">Preliminary opportunity screening</div></div></div><details style="margin-top:12px"><summary style="font-size:10px;color:rgba(255,255,255,.72);cursor:pointer">Score breakdown</summary><div class="score-bars">${scoreBar("Financial",c.financial_opportunity)}${scoreBar("Market",c.market_strength)}${scoreBar("Development",c.development_efficiency)}${scoreBar("Planning",c.planning_certainty)}${scoreBar("Site / parking",c.site_constraints)}</div></details>`;
...
async function selectSite(lat,lon){
 ...
 try{
   ...
   const br=await fetch(`/api/parcel-basic?lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}`,{signal}),basic=await br.json();if(!br.ok)throw new Error(basic.detail||"Parcel lookup failed");if(requestId!==selectionRequestId)return;
   ...
   const dr=await fetch(`/api/parcel-details?parcel_id=${encodeURIComponent(basic.parcel_id)}`,{signal}),details=await dr.json();if(!dr.ok)throw new Error(details.detail||"Parcel details failed");if(requestId!==selectionRequestId)return;
...
map.on("click",e=>{if(map.getZoom()<15){alert("Zoom in further before selecting a parcel.");return}selectSite(e.latlng.lat,e.latlng.lng)});
...
function renderOpportunityResults(d){
 const panel=$("opportunityPanel");panel.classList.remove("hidden");panel.innerHTML=`<div class="opp-head"><div><div class="eyebrow">PLANA OPPORTUNITY FINDER</div><h2>${esc(d.results?.length||0)} opportunities surfaced</h2></div><button class="close" id="closeOpp">×</button></div><div class="opp-summary">${esc(d.scanned_parcels)} DLS parcels screened in the visible map area · ${esc(d.standout_count)} stand out. ${d.budget_status==='tested_on_open'?'Budget is tested precisely when a parcel is opened.':''}</div>${(d.results||[]).slice(0,8).map((x,i)=>`<div class="opp-card" data-i="${i}"><div class="opp-rank"><div><div class="eyebrow">#${i+1} · ${esc(x.score_label)}</div><h3>Parcel ${esc(x.parcel_number||x.parcel_id)} · ${esc(x.quarter||x.municipality||x.district||"Cyprus")}</h3></div><div class="opp-score">${esc(x.score)}</div></div><p>${esc(x.best_use)}${x.units_low!=null?` · ${esc(x.units_low)}–${esc(x.units_high)} indicative units`:""} · ${fmt(x.floor_capacity_m2,0)} m² floor capacity</p>${x.underused_percent!=null?`<div class="opp-why">${esc(x.underused_percent)}% preliminary development gap</div>`:""}${(x.why||[]).slice(0,2).map(w=>`<div class="opp-why">✓ ${esc(w)}</div>`).join("")}</div>`).join("")}`;
...
async function runFinder(){
 ...
 try{const d=await post("/api/find-opportunities",{...finderBrief,west:b.getWest(),south:b.getSouth(),east:b.getEast(),north:b.getNorth(),max_results:10},finderController.signal);$("finder").classList.add("hidden");renderOpportunityResults(d)}
...
$("searchForm").addEventListener("submit",async e=>{e.preventDefault();const q=$("searchInput").value.trim();if(!q)return;const r=await fetch(`/api/geocode?q=${encodeURIComponent(q)}`),d=await r.json();$("results").innerHTML="";(d.results||[]).slice(0,6).forEach(x=>{const b=document.createElement("button");b.type="button";b.className="result";b.textContent=x.display_name;b.onclick=()=>{map.setView([x.lat,x.lon],18);$("results").innerHTML=""};$("results").appendChild(b)})});
$("askAiBtn").onclick=async()=>{const q=$("aiQuestion").value.trim();if(!q||!currentSite)return;const box=$("aiAnswer");box.classList.remove("hidden");box.textContent="Thinking…";try{const d=await post("/api/parcel-ai",{question:q,parcel_context:{parcel:currentSite.parcel,planning_zones:currentSite.planning_zones,development_potential:currentSite.development_potential,structured_rule_analysis:currentSite.structured_rule_analysis,automatic_planning_analysis:currentPlanning,development_options:currentProposals?.development_options,decision_intelligence:currentDecision},scenario:{}});box.textContent=d.answer||"No answer returned."}catch(e){box.textContent=`Could not answer: ${e.message||e}`}};
