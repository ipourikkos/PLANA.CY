...
async def site(
    ...
    # Slow parcel-wide GIS checks are loaded separately by /api/site-extra.
    spatial_checks = {}
...
""".strip()
    ...
    SITE_CACHE[sbpi] = (time.time(), result_payload)
...
def generate_configurations(payload: ConfigurationRequest) -> dict[str, Any]:
    context = payload.parcel_context or {}
    parcel = context.get("parcel") or {}
    zones = context.get("planning_zones") or []
    potential = context.get("development_potential") or {}

    cache_key = json.dumps(
        {
            "parcel_id": parcel.get("parcel_id"),
            "parcel_number": parcel.get("parcel_number"),
            "area": parcel.get("parcel_extent_m2"),
            "zones": zones,
            "potential": potential,
        },
        sort_keys=True,
    ...
    prompt = f"""
You are generating early-stage architectural massing/configuration options for a Cyprus development parcel.

PARCEL FACTS:
{json.dumps(parcel, ensure_ascii=False, indent=2)}

PLANNING ZONES:
{json.dumps(zones, ensure_ascii=False, indent=2)}

CALCULATED DEVELOPMENT POTENTIAL:
{json.dumps(potential, ensure_ascii=False, indent=2)}

Return 3 to 4 distinct, realistic concept configurations that stay within the known parcel-level planning limits.

For each configuration provide:
- name
- concept in one short sentence
- floors
- approximate total floor area in m²
- approximate ground footprint in m²
- approximate floor area per floor in m²
- why this configuration may suit the parcel
- key caveat

Rules:
- Do not invent setbacks, parking rules, use permissions, road access, fire requirements, unit counts, basement rights, or legal entitlements.
- Do not exceed the known maximum floors.
- Do not exceed the theoretical maximum floor area.
- Do not exceed the theoretical maximum ground coverage.
- If the parcel has multiple zones, preserved/ancient status, existing buildings, state-land overlap, coast-protection overlap, or any other flagged constraint, reflect that uncertainty in the caveats.
- These are concept configurations only, not planning approvals or guaranteed development rights.
- Return concise JSON only in this exact shape:
{{
  "configurations": [
    {{
      "name": "...",
      ...
      "ground_footprint_m2": 0,
      "floor_area_per_floor_m2": 0,
      "why_it_may_suit": "...",
      "key_caveat": "..."
    }}
  ]
...
""".strip()
    ...
    try:
        ...
        if not isinstance(data, dict) or not isinstance(data.get("configurations"), list):
            raise ValueError("Unexpected AI configuration format.")
    except Exception as exc:
        raise HTTPException(
            status_code=502,
            detail=f"Could not generate concept configurations: {exc}",
        )

    result = {
        "configurations": data["configurations"],
        "disclaimer": "AI-generated concept configurations based only on currently known parcel-level data. They are not planning approvals or guaranteed development rights.",
    }
  ...
  <div class="panel-scroll">
    ...
    <section class="section" id="aiSection">
      ...
      <div class="ai-box">
        ...
        <div id="aiAnswer" class="ai-answer muted" style="margin-top:10px">Select a parcel first.</div>
      </div>
    </section>
...
<div class="map-wrap">
      <div id="loadingPanel" class="loading-panel">
        <div id="loadParcel" class="load-step">Finding parcel…</div>
        <div id="loadPlanning" class="load-step">Loading planning data…</div>
      </div>
      <div id="map"></div>
    </div>
</div>
...
function setLoadingStage(stage){
 const ids=["loadParcel","loadPlanning"];
...
function resetSite(){
 currentSite=null; currentConfigurations=[]; currentAiAnswer="";
...
async function loadConfigurations(){
 ...
 $("configurations").innerHTML=configSkeleton()+'<div class="muted" style="margin-top:8px">Generating concept configurations…</div>';
 try{
   const r=await fetch("/api/configurations",{
     method:"POST",
     headers:{"Content-Type":"application/json"},
     body:JSON.stringify({parcel_context:buildParcelContext()})
   });
   const d=await r.json();
   if(!r.ok)throw new Error(d.detail||"Configuration generation failed");
   currentConfigurations=d.configurations||[];
   renderConfigurations(currentConfigurations);
 }catch(err){
   $("configurations").innerHTML=`<div class="warning">${esc(err.message||err)}</div>`;
 }
}
...
async function loadSiteExtras(){
 if(!currentSite)return;
 $("terrain").innerHTML='<div class="muted">Loading building and terrain data…</div>';
 $("spatial").innerHTML='<div class="muted">Checking site constraints…</div>';
 try{
   const r=await fetch("/api/site-extra",{
     method:"POST",
     headers:{"Content-Type":"application/json"},
     body:JSON.stringify({parcel_feature:currentSite.parcel_feature})
   });
   const d=await r.json();
   if(!r.ok)throw new Error(d.detail||"Extra site checks failed");

   currentSite.spatial_checks=d.spatial_checks||{};
   currentSite.building_summary=d.building_summary||{count:0,features:[]};
   currentSite.contour_summary=d.contour_summary||{};

   const b=currentSite.building_summary||{};
   const c=currentSite.contour_summary||{};
   $("terrain").innerHTML=`<div class="grid">
     ${card("Mapped buildings",b.count,true)}
     ${card("Contour lines",c.count,true)}
     ${card("Minimum contour elevation",c.min_elevation_m,false," m")}
     ${card("Maximum contour elevation",c.max_elevation_m,false," m")}
     ${card("Approx. elevation range",c.elevation_range_m,false," m")}
   </div>`;

   $("spatial").innerHTML=Object.entries(currentSite.spatial_checks||{}).map(([id,x])=>{
   ...
   $("terrain").innerHTML='<div class="muted">Building and terrain data could not be loaded.</div>';
   $("spatial").innerHTML='<div class="muted">Site constraints could not be loaded.</div>';
 }
}
...
async function selectSite(lat,lon){
 ...
 $("configurations").innerHTML=configSkeleton();
 $("configurationComparison").innerHTML="";
 ...
 const d=await r.json();
 if(!r.ok){hideLoading();alert(d.detail||"Lookup failed");return}
 setLoadingStage(1);
 currentSite=d;
 currentAiAnswer='';
 currentConfigurations=[];
 updateStickyParcel(d);
 $("aiContext").textContent=`Asking about Parcel ${d.parcel?.parcel_number||"—"} · ${d.planning_zones?.[0]?.zone||"zone not returned"} · ${d.parcel?.parcel_extent_m2||"—"} m²`;
