...
# Economy mode removes optional query-expansion and LLM-reranking passes.
# Legal-answer verification stays enabled by default and can be disabled only
# through its explicit environment switch.
AI_ECONOMY_MODE = os.getenv("PLANA_AI_ECONOMY_MODE", "true").strip().lower() == "true"
QUERY_EXPANSION_ENABLED = os.getenv(
    "PLANA_QUERY_EXPANSION_ENABLED", "false" if AI_ECONOMY_MODE else "true"
).strip().lower() == "true"
LLM_RERANK_ENABLED = os.getenv(
    "PLANA_LLM_RERANK_ENABLED", "false" if AI_ECONOMY_MODE else "true"
).strip().lower() == "true"
ANSWER_VERIFICATION_ENABLED = os.getenv(
    "PLANA_ANSWER_VERIFICATION_ENABLED", "true"
).strip().lower() == "true"
PLANNING_VERIFICATION_ENABLED = os.getenv(
    "PLANA_PLANNING_VERIFICATION_ENABLED", "true"
).strip().lower() == "true"
...
def batch_semantic_candidates(
    ...
    # Preserve order while avoiding duplicate paid embeddings when query
    # expansion is disabled or returns the original wording.
    queries = list(dict.fromkeys(q.strip() for q in query_texts if q and q.strip()))
    if not queries:
        return []
...
def generate_greek_search_query(
    ...
    """
    if contains_greek(question) or not QUERY_EXPANSION_ENABLED:
        return question
...
"""
    ...
    if not PLANNING_VERIFICATION_ENABLED:
        result = normalise_planning_analysis(raw, source_map)
        result.update({
            "status": "complete" if result["material_provisions"] else "limited",
            "retrieval": retrieval_metrics,
            "analysis_engine_version": "planning-auto-v3-economy",
            "model_passes": 1,
            "elapsed_ms": round((time.perf_counter() - started) * 1000),
        })
        return result
...
async def lifespan(app: FastAPI):
    ...
    snapshot_rows = _load_zone_registry_snapshot()
    overrides = await asyncio.to_thread(_load_zone_registry_supabase)
    merged_registry = {(row.get("development_plan_key") or _registry_text(row.get("development_plan")), row.get("zone_key") or _registry_text(row.get("zone_code"))): row for row in snapshot_rows}
    for row in overrides:
        merged_registry[(row["development_plan_key"], row["zone_key"])] = row
    _install_zone_registry(list(merged_registry.values()), "bundled_official_snapshot" + ("+supabase_overrides" if overrides else ""))
    await _refresh_zone_registry_online()

    print(
        f"PLANA.CY ready — loaded {len(state['all_rows'])} knowledge-base chunks and "
        f"{len(state.get('zone_registry_records') or [])} verified zone parameter rows."
    )
...
def health() -> dict[str, Any]:
    ...
    return {
        ...
        "live_dls_first": LIVE_DLS_FIRST,
        "live_market_enabled": LIVE_MARKET_ENABLED,
        "google_places_configured": bool(os.getenv("GOOGLE_MAPS_API_KEY")),
        "zone_registry_rows": len(state.get("zone_registry_records") or []),
        "zone_registry_source": state.get("zone_registry_source"),
        "model": ANSWER_MODEL,
        "rule_engine_version": RULE_ENGINE_VERSION,
...
def chat(payload: ChatRequest) -> dict[str, Any]:
    ...
    cache_key = (APP_BUILD, len(all_rows), normalize_text(question))
    cached = CHAT_RESPONSE_CACHE.get(cache_key)
    if cached and time.time() - cached[0] < AI_RESPONSE_CACHE_TTL_SECONDS:
        return {**cached[1], "cached": True}

    try:
        greek_search_query = generate_greek_search_query(question, openai_client)

        semantic_rows = batch_semantic_candidates(
            [question, greek_search_query],
            openai_client,
            supabase,
        )
        ...
        CHAT_RESPONSE_CACHE[cache_key] = (time.time(), result)
        return result
...
DEVELOPMENT_PLAN_QUERY = f"{DLS_MAPSERVER}/11/query"
OFFICIAL_ZONE_LEGEND_URL = os.getenv(
    "PLANA_ZONE_LEGEND_XLSX_URL",
    "https://www.data.gov.cy/sites/default/files/%CE%A5%CF%80%CF%8C%CE%BC%CE%BD%CE%B7%CE%BC%CE%B1%20%CE%A0%CE%BF%CE%BB%CE%B5%CE%BF%CE%B4%CE%BF%CE%BC%CE%B9%CE%BA%CF%8E%CE%BD%20%CE%96%CF%89%CE%BD%CF%8E%CE%BD%20%CF%84%CE%B7%CF%82%20%CE%9A%CF%85%CF%80%CF%81%CE%B9%CE%B1%CE%BA%CE%AE%CF%82%20%CE%94%CE%B7%CE%BC%CE%BF%CE%BA%CF%81%CE%B1%CF%84%CE%AF%CE%B1%CF%82.xlsx",
)
ZONE_REGISTRY_SNAPSHOT = Path(__file__).with_name("planning_zone_parameters.json")
APP_BUILD = "2026-07-18-prime-villa-whole-property-v9"
LIVE_DLS_FIRST = os.getenv("PLANA_LIVE_DLS_FIRST", "true").strip().lower() == "true"
LIVE_MARKET_ENABLED = os.getenv("PLANA_LIVE_MARKET_ENABLED", "true").strip().lower() == "true"
# A single cost-efficient model now researches and returns structured market data.
MARKET_RESEARCH_MODEL = os.getenv("OPENAI_MARKET_RESEARCH_MODEL", os.getenv("OPENAI_CHEAP_MODEL", "gpt-5-mini"))
MARKET_RESEARCH_TIMEOUT_SECONDS = max(30, int(os.getenv("PLANA_MARKET_RESEARCH_TIMEOUT_SECONDS", "105")))
ZONE_REGISTRY_REFRESH_SECONDS = max(3600, int(os.getenv("PLANA_ZONE_REGISTRY_REFRESH_SECONDS", "86400")))
...
def _registry_clean(value: Any) -> str | None:
    if value is None:
        return None
...
def _registry_numbers(value: Any) -> list[float]:
    if value in (None, ""):
        return []
...
def _registry_ratio(value: Any) -> float | None:
    if value in (None, ""):
        return None
    ...
    try:
        ...
        return None
    return round(number * 100.0 if abs(number) <= 5 else number, 4)


def _registry_lines(value: Any) -> list[str]:
    return [
...
def _registry_tags(value: str) -> list[str]:
...
def _zone_use_matrix(zone_code: str, description: str | None) -> dict[str, str]:
    zone = str(zone_code or "").strip()
    desc = _registry_text(description)
    result = {
        "house": "not_supported", "apartments": "not_supported",
        "office": "not_supported", "shop": "not_supported",
        "mixed_use": "not_supported", "tourist": "not_supported",
        "industrial": "not_supported",
    }
    if zone.startswith(("Κα", "ΚΑ", "Η")) or "κατοικ" in desc or "οικιστικ" in desc:
        result["house"] = result["apartments"] = "allowed"
    elif zone.startswith(("Ε", "Εβ", "Εμ")) or "εμπορ" in desc:
        result.update({"office": "allowed", "shop": "allowed", "mixed_use": "allowed", "apartments": "conditional", "house": "conditional"})
    elif zone.startswith(("Τ", "Τβ")) or "τουρισ" in desc:
        result.update({"tourist": "allowed", "house": "conditional", "apartments": "conditional", "shop": "conditional"})
    elif zone.startswith(("Β", "Βα", "Ββ")) or "βιομηχαν" in desc or "βιοτεχν" in desc:
        result.update({"industrial": "allowed", "office": "conditional"})
    elif zone.startswith("Πα") or "πυρην" in desc or "συνεχουσδομησ" in desc:
        result.update({"house": "allowed", "apartments": "conditional", "office": "conditional", "shop": "conditional", "mixed_use": "conditional"})
    elif zone.startswith("Γ") or "γεωργ" in desc or "κτηνοτροφ" in desc:
        result.update({"house": "conditional", "industrial": "conditional"})
    return result


def _parse_zone_registry_row(
    zone: Any, description: Any, density_raw: Any, floors_raw: Any,
    height_raw: Any, coverage_raw: Any, special: Any, plan: Any,
) -> dict[str, Any] | None:
    zone_text = _registry_clean(zone)
    plan_text = _registry_clean(plan)
    description_text = _registry_clean(description)
    if not zone_text or not plan_text or _registry_text(zone_text) in {"πολεοδομικηζωνη", "σημειωσεισ"}:
        return None
    if not any(_registry_numbers(value) for value in (density_raw, floors_raw, height_raw, coverage_raw)):
        return None

    density_lines = _registry_lines(density_raw)
    floors_lines = _registry_lines(floors_raw)
    height_lines = _registry_lines(height_raw)
    coverage_lines = _registry_lines(coverage_raw)
    # Some Policy Statement sections place coverage before floors and height.
    if any(":1" in item.replace(" ", "") for item in floors_lines):
        coverage_lines, floors_lines, height_lines = floors_lines, height_lines, coverage_lines

    count = max(len(density_lines), len(coverage_lines), len(floors_lines), len(height_lines), 1)
    variants: list[dict[str, Any]] = []
    for index in range(count):
        density_line = density_lines[index] if index < len(density_lines) else (density_lines[-1] if density_lines else "")
        coverage_line = coverage_lines[index] if index < len(coverage_lines) else (coverage_lines[-1] if coverage_lines else "")
        floors_line = floors_lines[index] if index < len(floors_lines) else (floors_lines[-1] if floors_lines else "")
        height_line = height_lines[index] if index < len(height_lines) else (height_lines[-1] if height_lines else "")
        floor_numbers = _registry_numbers(floors_line)
        height_numbers = _registry_numbers(height_line)
        variants.append({
            "use_tags": _registry_tags(density_line) or _registry_tags(coverage_line),
            "density_percent": _registry_ratio(density_line),
            "coverage_percent": _registry_ratio(coverage_line),
            "max_floors": max(floor_numbers) if floor_numbers else None,
            "max_height_m": max(height_numbers) if height_numbers else None,
        })

    def residential_score(item: dict[str, Any]) -> float:
        tags = _registry_text(" ".join(item.get("use_tags") or []))
        score = 10 if any(token in tags for token in ("κ", "κατ", "κατοικ", "οδ")) else 0
        return score + sum(item.get(key) is not None for key in ("density_percent", "coverage_percent", "max_floors", "max_height_m"))

    selected = max(variants, key=residential_score)
    return {
        "development_plan": plan_text,
        "development_plan_key": _registry_text(plan_text),
        "zone_code": zone_text,
        "zone_key": _registry_text(zone_text),
        "description": description_text,
        "density_percent": selected.get("density_percent"),
        "coverage_percent": selected.get("coverage_percent"),
        "max_floors": selected.get("max_floors"),
        "max_height_m": selected.get("max_height_m"),
        "special_provisions": _registry_clean(special),
        "allowed_uses": _zone_use_matrix(zone_text, description_text),
        "variants": variants,
        "source_title": "Υπόμνημα Χάρτη Πολεοδομικών Ζωνών - Μάρτιος 2016",
        "source_url": "https://www.data.gov.cy/el/dataset/hartis-poleodomikon-zonon-tis-kypriakis-dimokratias",
    }


def _parse_zone_registry_xlsx(content: bytes) -> list[dict[str, Any]]:
    workbook = load_workbook(BytesIO(content), read_only=True, data_only=True)
    parsed: list[dict[str, Any]] = []
    sheet1 = workbook[workbook.sheetnames[0]]
    for row in sheet1.iter_rows(min_row=2, values_only=True):
        values = list(row) + [None] * 8
        item = _parse_zone_registry_row(*values[:8])
        if item:
            parsed.append(item)
    if len(workbook.sheetnames) > 1:
        ...
        for row in sheet2.iter_rows(values_only=True):
            ...
            for columns in ((0, 1, 2, 3, 4, 5, 6, 7), (12, 13, 14, 16, 17, 15, None, 18)):
                args = [values[index] if index is not None else None for index in columns]
                item = _parse_zone_registry_row(*args)
                if item:
                    parsed.append(item)
    deduped: dict[tuple[str, str], dict[str, Any]] = {}
    for item in parsed:
        key = (item["development_plan_key"], item["zone_key"])
        quality = sum(item.get(field) is not None for field in ("density_percent", "coverage_percent", "max_floors", "max_height_m"))
        old = deduped.get(key)
        old_quality = sum(old.get(field) is not None for field in ("density_percent", "coverage_percent", "max_floors", "max_height_m")) if old else -1
        if quality > old_quality:
            deduped[key] = item
    return list(deduped.values())


def _install_zone_registry(records: list[dict[str, Any]], source_name: str) -> None:
    by_pair: dict[tuple[str, str], dict[str, Any]] = {}
    by_zone: dict[str, list[dict[str, Any]]] = {}
    for row in records:
        plan_key = row.get("development_plan_key") or _registry_text(row.get("development_plan"))
        zone_key = row.get("zone_key") or _registry_text(row.get("zone_code"))
        if not plan_key or not zone_key:
            continue
        row = {**row, "development_plan_key": plan_key, "zone_key": zone_key}
        by_pair[(plan_key, zone_key)] = row
        by_zone.setdefault(zone_key, []).append(row)
    state["zone_registry_records"] = list(by_pair.values())
    state["zone_registry_by_pair"] = by_pair
    state["zone_registry_by_zone"] = by_zone
    state["zone_registry_source"] = source_name
    state["zone_registry_loaded_at"] = datetime.now(timezone.utc).isoformat()


def _load_zone_registry_snapshot() -> list[dict[str, Any]]:
    try:
        payload = json.loads(ZONE_REGISTRY_SNAPSHOT.read_text(encoding="utf-8"))
        rows = payload.get("records") if isinstance(payload, dict) else payload
        return [dict(item) for item in rows or [] if isinstance(item, dict)]
    except Exception as exc:
        print(f"PLANA zone snapshot load failed: {exc!r}", file=sys.stderr)
        return []


def _load_zone_registry_supabase() -> list[dict[str, Any]]:
    supabase = state.get("supabase")
    if not supabase:
        return []
    try:
        response = supabase.table("planning_zone_parameters").select("*").eq("is_active", True).limit(5000).execute()
        output = []
        for row in response.data or []:
            output.append({
                "development_plan": row.get("development_plan"),
                "development_plan_key": _registry_text(row.get("development_plan")),
                "zone_code": row.get("zone_code"),
                "zone_key": _registry_text(row.get("zone_code")),
                ...
                "allowed_uses": row.get("allowed_uses") or _zone_use_matrix(str(row.get("zone_code") or ""), row.get("description")),
...
async def _refresh_zone_registry_online() -> None:
    client = state.get("http")
    if not client:
        return
    try:
        response = await client.get(OFFICIAL_ZONE_LEGEND_URL, timeout=35.0)
        response.raise_for_status()
        rows = await asyncio.to_thread(_parse_zone_registry_xlsx, response.content)
        if len(rows) >= 400:
            overrides = await asyncio.to_thread(_load_zone_registry_supabase)
            merged = {(row["development_plan_key"], row["zone_key"]): row for row in rows}
            for row in overrides:
                merged[(row["development_plan_key"], row["zone_key"])] = row
            _install_zone_registry(list(merged.values()), "official_online_xlsx" + ("+supabase_overrides" if overrides else ""))
    except Exception as exc:
        print(f"PLANA official zone registry refresh warning: {exc!r}", file=sys.stderr)


def _district_plan_hint(district: Any) -> str:
    text = _registry_text(district)
    mapping = {
        "limassol": "λεμεσου", "lemesos": "λεμεσου", "λεμεσοσ": "λεμεσου",
        "nicosia": "λευκωσια", "lefkosia": "λευκωσια", "λευκωσια": "λευκωσια",
        "larnaca": "λαρνακα", "larnaka": "λαρνακα", "λαρνακα": "λαρνακα",
        "paphos": "παφου", "pafos": "παφου", "παφοσ": "παφου",
        "famagusta": "δερυνεια", "ammochostos": "δερυνεια", "αμμοχωστοσ": "δερυνεια",
    }
    return mapping.get(text, text)


def resolve_zone_parameter(development_plan: Any, zone_code: Any, district: Any = None) -> dict[str, Any] | None:
    zone_key = _registry_text(zone_code)
    if not zone_key:
        return None
    candidates = list((state.get("zone_registry_by_zone") or {}).get(zone_key) or [])
    if not candidates:
        return None
    plan_key = _registry_text(development_plan)
    if plan_key:
        exact = (state.get("zone_registry_by_pair") or {}).get((plan_key, zone_key))
        if exact:
            return {**exact, "resolution": "exact_plan_zone"}
        best = max(candidates, key=lambda row: SequenceMatcher(None, plan_key, row.get("development_plan_key") or "").ratio())
        score = SequenceMatcher(None, plan_key, best.get("development_plan_key") or "").ratio()
        if score >= 0.42:
            return {**best, "resolution": "fuzzy_plan_zone", "resolution_score": round(score, 3)}
    hint = _district_plan_hint(district)
    if hint:
        hinted = [row for row in candidates if hint in (row.get("development_plan_key") or "")]
        if hinted:
            return {**hinted[0], "resolution": "district_zone"}
    dimensions = {
        (row.get("density_percent"), row.get("coverage_percent"), row.get("max_floors"), row.get("max_height_m"))
        for row in candidates
    }
    if len(dimensions) == 1:
        return {**candidates[0], "resolution": "cross_plan_consensus"}
    # Last resort: choose the most conservative complete record rather than returning blanks.
    complete = [row for row in candidates if all(row.get(field) is not None for field in ("density_percent", "coverage_percent", "max_floors", "max_height_m"))]
    if complete:
        chosen = min(complete, key=lambda row: (float(row.get("density_percent") or 999), float(row.get("coverage_percent") or 999)))
        return {**chosen, "resolution": "conservative_zone_fallback"}
    return candidates[0]
...
def enrich_parcel_planning(details: dict[str, Any]) -> dict[str, Any]:
    parcel = details.get("parcel") or {}
    development_plan = parcel.get("development_plan") or (details.get("development_plan") or {}).get("name")
    enriched: list[dict[str, Any]] = []
    for zone in details.get("planning_zones") or []:
        record = resolve_zone_parameter(development_plan, zone.get("zone"), parcel.get("district"))
        item = dict(zone)
        if record:
            for field in ("density_percent", "coverage_percent", "max_floors", "max_height_m"):
                if _usable_number(item.get(field)) is None and _usable_number(record.get(field)) is not None:
                    item[field] = record.get(field)
            item["description"] = item.get("description") or record.get("description")
            item["description_en"] = item.get("description_en") or record.get("description")
            item["allowed_uses"] = record.get("allowed_uses") or _zone_use_matrix(str(item.get("zone") or ""), item.get("description"))
            item["parameter_source"] = {
                "registry": state.get("zone_registry_source"),
                "development_plan": record.get("development_plan"),
                "resolution": record.get("resolution"),
            }
        else:
            item["allowed_uses"] = _zone_use_matrix(str(item.get("zone") or ""), item.get("description") or item.get("remarks"))
        enriched.append(item)
    details["planning_zones"] = enriched

    area = _usable_number(parcel.get("parcel_extent_m2"))
    capacity = calculate_zoned_capacity(area, enriched)
    if area and enriched and capacity.get("theoretical_max_floor_area_m2") is None:
        densities = [_usable_number(item.get("density_percent")) for item in enriched]
        coverages = [_usable_number(item.get("coverage_percent")) for item in enriched]
        densities = [value for value in densities if value is not None]
        coverages = [value for value in coverages if value is not None]
        if densities and coverages:
            density = min(densities)
            coverage = min(coverages)
            capacity.update({
                "calculation_method": "multi_zone_conservative_coefficients",
                "effective_density_percent": density,
                "effective_coverage_percent": coverage,
                "theoretical_max_floor_area_m2": round(area * density / 100.0, 2),
                "theoretical_max_ground_coverage_m2": round(area * coverage / 100.0, 2),
                "zone_overlap_complete": False,
            })
    details["development_potential"] = {
        **(details.get("development_potential") or {}),
        **capacity,
        "calculation_authority_status": "official_dls_geometry_plus_verified_zone_registry",
        "zone_registry_source": state.get("zone_registry_source"),
    }
    details["structured_rule_analysis"] = evaluate_parcel_rules(details)
    return details
...
LOCATION_INTELLIGENCE_CACHE_TTL_SECONDS = max(900, int(os.getenv("PLANA_NEARBY_CACHE_TTL_SECONDS", "21600")))
NEARBY_RADIUS_M = max(300, min(3000, int(os.getenv("PLANA_NEARBY_RADIUS_M", "1500"))))
NEARBY_PLACES_PER_CATEGORY = max(1, min(10, int(os.getenv("PLANA_NEARBY_PLACES_PER_CATEGORY", "5"))))
...
MARKET_ANALYSIS_CACHE_TTL_SECONDS = max(3600, int(os.getenv("PLANA_MARKET_RESEARCH_CACHE_TTL_SECONDS", "604800")))
MARKET_ESTIMATE_CACHE_TABLE = os.getenv("PLANA_MARKET_ESTIMATE_CACHE_TABLE", "plana_market_estimate_cache")
...
PARCEL_PLANNING_CACHE_TTL_SECONDS = max(3600, int(os.getenv("PLANA_PLANNING_CACHE_TTL_SECONDS", "86400")))
CHAT_RESPONSE_CACHE = BoundedCache(max(128, WEB_CACHE_MAX_ENTRIES // 2))
PARCEL_AI_RESPONSE_CACHE = BoundedCache(max(128, WEB_CACHE_MAX_ENTRIES // 2))
AI_RESPONSE_CACHE_TTL_SECONDS = max(900, int(os.getenv("PLANA_AI_RESPONSE_CACHE_TTL_SECONDS", "86400")))
...
[out:json][timeout:14];
(
  nwr(around:{NEARBY_RADIUS_M},{lat},{lon})["shop"];
  nwr(around:{NEARBY_RADIUS_M},{lat},{lon})["amenity"~"restaurant|cafe|fast_food|bar|pub|food_court|school|college|university|kindergarten|pharmacy|hospital|clinic|doctors|dentist|place_of_worship|bus_station"];
  nwr(around:{NEARBY_RADIUS_M},{lat},{lon})["leisure"~"park|garden|playground|nature_reserve|fitness_centre|sports_centre|stadium|swimming_pool"];
  nwr(around:{NEARBY_RADIUS_M},{lat},{lon})["tourism"~"hotel|attraction|museum"];
  nwr(around:{NEARBY_RADIUS_M},{lat},{lon})["natural"="beach"];
  nwr(around:{NEARBY_RADIUS_M},{lat},{lon})["public_transport"];
  node(around:{NEARBY_RADIUS_M},{lat},{lon})["highway"="bus_stop"];
...
""".strip()
    ...
    for item in overpass.get("elements") or []:
        ...
        if not category or distance_km * 1000 > NEARBY_RADIUS_M:
            continue
    ...
    for category in order:
        ...
        if not rows:
            continue
        nearest_rows = rows[:NEARBY_PLACES_PER_CATEGORY]
        amenities.append({
            "category": category,
            "label": labels[category],
            "count": len(nearest_rows),
            "total_within_radius": len(rows),
            "radius_cap_m": NEARBY_RADIUS_M,
            "nearest_distance_m": nearest_rows[0]["distance_m"],
            "nearest_name": nearest_rows[0]["name"],
            "places": nearest_rows,
        })
    ...
    result = {
        ...
        "radius_cap_m": NEARBY_RADIUS_M,
        "places_per_category": NEARBY_PLACES_PER_CATEGORY,
        "cached": False,
    }
...
async def query_layer_intersections(layer_id: int, parcel_feature: dict, *, return_geometry: bool = False):
...
async def parcel_basic(
    ...
    if LIVE_DLS_FIRST:
        try:
            ...
            parcel_id = int(props.get("SBPI_ID_NO"))
            return {
    ...
    if indexed:
        ...
        if parcel_feature:
            parcel_feature["properties"] = {"SBPI_ID_NO": indexed.get("parcel_id"), "PARCEL_NBR": indexed.get("parcel_number")}
            return {
                "parcel_feature": parcel_feature,
                ...
                "geometry_metrics": {
                    "approx_area_m2": indexed.get("parcel_area_m2"), "perimeter_m": indexed.get("perimeter_m"),
                    "longest_edge_m": indexed.get("longest_edge_m"), "shortest_edge_m": indexed.get("shortest_edge_m"),
                    "centroid_lat": indexed.get("centroid_lat"), "centroid_lon": indexed.get("centroid_lon"),
                },
                "map_layer_attributes": parcel_feature["properties"],
                "source": "supabase_cache_fallback",
            }
    if live_error:
        raise HTTPException(status_code=502, detail=f"Official DLS parcel lookup failed: {live_error}")
    raise HTTPException(status_code=404, detail="No parcel was found at that point.")
...
async def get_arcgis_parcel_feature_by_id(parcel_id: int) -> dict[str, Any] | None:
    ...
    try:
        r = await client.get(PARCEL_QUERY, params=params, timeout=20.0)
        if r.status_code != 200:
            return None
        data = r.json()
...
async def canonical_details_from_arcgis(parcel_id: int) -> dict[str, Any] | None:
    """Online-first canonical parcel analysis from official DLS ArcGIS layers."""
    ...
    results = await asyncio.gather(
        query_layer_intersections(11, feature, return_geometry=False),
        query_layer_intersections(12, feature, return_geometry=True),
        query_layer_intersections(15, feature),
        query_layer_intersections(16, feature),
        query_layer_intersections(17, feature),
        query_layer_intersections(28, feature),
        query_layer_intersections(31, feature),
        query_layer_intersections(32, feature),
        query_layer_intersections(37, feature),
        return_exceptions=False,
    )
    (plan_result, zone_result, district_result, municipality_result, quarter_result,
     building_result, coast_result, state_land_result, white_zone_result) = results

    def features(result: dict[str, Any]) -> list[dict[str, Any]]:
        return list(result.get("features") or []) if isinstance(result, dict) and result.get("ok") else []

    def attrs(result: dict[str, Any]) -> list[dict[str, Any]]:
        return [(item.get("attributes") or {}) for item in features(result)]

    plan_attrs = attrs(plan_result)
    development_plan = _registry_clean(plan_attrs[0].get("DEVP_NM_G")) if plan_attrs else None
    parcel_shape = shape(feature.get("geometry") or {})
    parcel_planar_area = parcel_shape.area if not parcel_shape.is_empty else 0.0
    zones: list[dict[str, Any]] = []
    seen: set[str] = set()
    for zone_feature in features(zone_result):
        attributes = zone_feature.get("attributes") or {}
        name = _registry_clean(attributes.get("PLNZNT_NAME"))
        ...
        if rings and parcel_planar_area > 0:
            try:
                zone_shape = shape({"type": "Polygon", "coordinates": rings})
                overlap = round(max(0.0, min(100.0, zone_shape.intersection(parcel_shape).area / parcel_planar_area * 100.0)), 3)
            except Exception:
                overlap = None
        zones.append({
            "zone": name,
            "description": _registry_clean(attributes.get("PLNZNT_DESC")),
            "remarks": _registry_clean(attributes.get("PLNZNT_DESC")),
            "overlap_percent": overlap,
            "density_percent": None, "coverage_percent": None, "max_floors": None, "max_height_m": None,
        })

    district_attrs, municipality_attrs, quarter_attrs = attrs(district_result), attrs(municipality_result), attrs(quarter_result)
    parcel_area = props.get("SHAPE.STArea()") or props.get("Parcel Extend") or metrics.get("approx_area_m2")
    result: dict[str, Any] = {
        "parcel": {
            "parcel_id": parcel_id, "parcel_number": props.get("PARCEL_NBR"), "registration_number": None,
            "district": _registry_clean(district_attrs[0].get("DIST_NM_E")) if district_attrs else None,
            "municipality": _registry_clean(municipality_attrs[0].get("VIL_NM_E")) if municipality_attrs else None,
            "quarter": _registry_clean(quarter_attrs[0].get("QRTR_NM_E")) if quarter_attrs else None,
            "development_plan": development_plan,
            "sheet": _registry_clean(props.get("SHEET")), "plan": _registry_clean(props.get("PLAN_NBR")),
            "block": props.get("BLCK_CODE"), "scale": props.get("SRC_SL_CODE"), "postal_code": None, "house_no": None,
            "parcel_extent_m2": parcel_area, "price_2021": None, "price_2018": None, "price_1980": None,
            "valuation_change_percent": None, "is_preserved": False, "is_ancient": False, "is_common_property": False,
        },
        "planning_zones": zones,
        "development_potential": {},
        "geometry_metrics": metrics,
        "registration_summary": {"total_related_records": 0, "by_type": {}, "total_enclosed_extent_m2": None, "total_covered_extent_m2": None, "total_uncovered_extent_m2": None},
        "related_properties": [],
        "warnings": [],
        "building_summary": {"count": len(features(building_result)), "features": []},
        "contour_summary": {},
        "spatial_checks": {
            "coast_protection": bool(features(coast_result)),
            "state_land": bool(features(state_land_result)),
            "white_zone": bool(features(white_zone_result)),
        },
        "fallback_metadata": {"source": "official_dls_live", "detailed_identify_used": False},
    }
    return enrich_parcel_planning(result)


async def get_canonical_parcel_details(parcel_id: int) -> dict[str, Any]:
    cached = SITE_CACHE.get(parcel_id)
    now = time.time()
    if cached and now - cached[0] < min(SITE_CACHE_TTL_SECONDS, 600):
        return cached[1]
    ...
    if LIVE_DLS_FIRST:
        try:
            ...
            live_error = exc
            print(f"PLANA live DLS detail warning for {parcel_id}: {exc!r}", file=sys.stderr)

    indexed = await get_indexed_parcel_by_id(parcel_id)
    if indexed:
        result = enrich_parcel_planning(canonical_details_from_index(indexed))
        SITE_CACHE[parcel_id] = (time.time(), result)
        return result
    ...
    try:
        ...
        if isinstance(records, list) and records:
            result = enrich_parcel_planning(normalize_parcel_details(records, parcel_id))
            SITE_CACHE[parcel_id] = (time.time(), result)
            return result
    except Exception:
        pass
    raise HTTPException(status_code=502, detail=f"Official DLS parcel services are unavailable{': ' + str(live_error) if live_error else ''}.")
...
async def parcel_details(parcel_id: int = Query(gt=0)) -> dict[str, Any]:
    ...
    fallback_market = _apply_expert_micro_market_profile(_fallback_market_analysis(details), details)
    fallback_market = _reconcile_prime_villa_consistency(fallback_market)
    fallback_market = _calibrate_market_analysis_ranges(fallback_market)
    assumptions = merge_automatic_market_assumptions({}, fallback_market)
    initial_opportunity = analyse_parcel_opportunity(details, assumptions)
    initial_opportunity["market_analysis"] = fallback_market
    return {
        **details,
        "initial_market_analysis": fallback_market,
        "initial_proposals": build_viable_development_options(details, None, initial_opportunity),
        "app_build": APP_BUILD,
    }
...
class LiveMarketBand(BaseModel):
    sale_low_eur_m2: float = Field(gt=0)
    sale_high_eur_m2: float = Field(gt=0)
    rent_low_eur_m2_month: float = Field(gt=0)
    rent_high_eur_m2_month: float = Field(gt=0)


class LiveMarketResearchResult(BaseModel):
    # Land fields are current asking evidence. PLANA converts them to an
    # achievable screening value using the discount field below.
    land_sale_low_eur_m2: float = Field(gt=0)
    land_sale_high_eur_m2: float = Field(gt=0)
    land_asking_to_achievable_discount_percent: float = Field(ge=0, le=30)
    apartment: LiveMarketBand
    house: LiveMarketBand
    office: LiveMarketBand
    shop: LiveMarketBand

    # Generic construction band for apartment/office/shop screening.
    construction_cost_low_eur_m2: float = Field(gt=0)
    construction_cost_high_eur_m2: float = Field(gt=0)

    # A detached villa is not adequately costed or valued by ordinary house
    # €/m² alone. These fields are specific to a new single-house development
    # on a plot comparable to the subject parcel and exclude land and VAT.
    detached_house_construction_cost_low_eur_m2: float = Field(gt=0)
    detached_house_construction_cost_high_eur_m2: float = Field(gt=0)
    detached_house_external_works_low_eur: float = Field(ge=0)
    detached_house_external_works_high_eur: float = Field(ge=0)
    detached_house_total_build_cost_low_eur: float = Field(gt=0)
    detached_house_total_build_cost_high_eur: float = Field(gt=0)
    detached_house_total_sale_low_eur: float = Field(gt=0)
    detached_house_total_sale_high_eur: float = Field(gt=0)
    detached_house_total_rent_low_eur_month: float = Field(gt=0)
    detached_house_total_rent_high_eur_month: float = Field(gt=0)
    market_tier: str
    market_area_used: str
    evidence_date: str


class LiveLuxuryVillaResearchResult(BaseModel):
    """Focused whole-property evidence for prime single-villa sites."""

    land_total_low_eur: float = Field(gt=0)
    land_total_high_eur: float = Field(gt=0)
    total_build_cost_low_eur: float = Field(gt=0)
    total_build_cost_high_eur: float = Field(gt=0)
    completed_sale_low_eur: float = Field(gt=0)
    completed_sale_high_eur: float = Field(gt=0)
    monthly_rent_low_eur: float = Field(gt=0)
    monthly_rent_high_eur: float = Field(gt=0)
    comparable_land_count: int = Field(ge=0)
    comparable_sale_count: int = Field(ge=0)
    comparable_rent_count: int = Field(ge=0)
    confidence: str
    market_tier: str
    market_area_used: str
    evidence_date: str
...
class LiveLandResearchResult(BaseModel):
    """Focused land-only validation used only when the first market pass is internally inconsistent."""
...
def _market_district_key(value: Any) -> str:
    text = _registry_text(value)
    for key, tokens in {
        "limassol": ("limassol", "lemesos", "λεμεσοσ"),
        "nicosia": ("nicosia", "lefkosia", "λευκωσια"),
        "larnaca": ("larnaca", "larnaka", "λαρνακα"),
        "paphos": ("paphos", "pafos", "παφοσ"),
        "famagusta": ("famagusta", "ammochostos", "αμμοχωστοσ"),
    }.items():
...
# Expert calibration anchors are used only for narrowly identified micro-markets
# where PLANA has verified local professional evidence. They are not district-wide
# defaults and do not affect ordinary Germasogeia or Limassol plots.
EXPERT_MICROMARKET_PROFILES: tuple[dict[str, Any], ...] = (
    {
        "profile_id": "potamos_germasogeias_amathounta_prime_villas_v1",
        "district_tokens": ("limassol", "lemesos"),
        "locality_tokens": ("potamos germasogeias", "amathounta"),
        "road_tokens": ("ymittou", "imittou", "υμηττου", "pentelis", "penteli", "πεντελησ"),
        "minimum_plot_area_m2": 800.0,
        "land_total_eur": {"low": 2_000_000.0, "high": 3_000_000.0},
        "single_villa_total_build_cost_eur": {"low": 3_000_000.0, "high": 4_000_000.0},
        "single_villa_completed_sale_eur": {"low": 9_000_000.0, "high": 12_000_000.0},
        "single_villa_monthly_rent_eur": {"low": 35_000.0, "high": 45_000.0},
        "market_tier": "ultra_prime_luxury_villa",
        "basis": "local_expert_calibration_and_immediate_luxury_villa_market",
    },
)


def _normalised_market_text(*values: Any) -> str:
...
def _matching_expert_micro_market_profile(parcel_details: dict[str, Any]) -> dict[str, Any] | None:
    parcel = parcel_details.get("parcel") or {}
    context = parcel_details.get("_market_context") or {}
    area = _usable_number(parcel.get("parcel_extent_m2")) or 0.0
    locality_text = _normalised_market_text(
        parcel.get("quarter"), parcel.get("municipality"), context.get("neighbourhood"),
        context.get("display_name"), context.get("formatted_address"),
    )
    district_text = _normalised_market_text(parcel.get("district"))
    road_text = _normalised_market_text(*(context.get("nearby_roads") or []), context.get("road"))
    for profile in EXPERT_MICROMARKET_PROFILES:
        if area < float(profile.get("minimum_plot_area_m2") or 0.0):
            continue
        district_match = any(token in district_text for token in profile.get("district_tokens") or ())
        locality_hits = sum(1 for token in profile.get("locality_tokens") or () if token in locality_text)
        road_match = any(token in road_text for token in profile.get("road_tokens") or ())
        # Match the exact named micro-market, one of its verified adjacent
        # roads, or a large site explicitly inside Potamos Germasogeias.
        large_potamos_site = (
            area >= 1_000.0
            and "potamos germasogeias" in locality_text
            and "germasogeia" in locality_text
        )
        if district_match and (locality_hits >= 2 or road_match or large_potamos_site):
            return dict(profile)
    return None


def _merge_anchor_range(
    current: dict[str, Any] | None,
    anchor: dict[str, Any],
    *,
    replace_below_ratio: float = 0.72,
) -> dict[str, float]:
    anchor_low = float(anchor["low"])
    anchor_high = float(anchor["high"])
    current_low = _usable_number((current or {}).get("low"))
    current_high = _usable_number((current or {}).get("high"))
    if current_low is None or current_high is None:
        return {"low": round(anchor_low, 2), "high": round(anchor_high, 2)}
    current_low, current_high = min(current_low, current_high), max(current_low, current_high)
    current_mid = (current_low + current_high) / 2.0
    anchor_mid = (anchor_low + anchor_high) / 2.0
    if current_mid < anchor_mid * replace_below_ratio:
        return {"low": round(anchor_low, 2), "high": round(anchor_high, 2)}
    # When live evidence is already in the calibrated market, retain it but do
    # not let a narrow model range erase the professionally observed anchor.
    return {
        "low": round(max(min(current_low, anchor_low), anchor_low * 0.90), 2),
        "high": round(max(current_high, anchor_high), 2),
    }


def _apply_expert_micro_market_profile(
    analysis: dict[str, Any],
    parcel_details: dict[str, Any],
) -> dict[str, Any]:
    profile = _matching_expert_micro_market_profile(parcel_details)
    if not profile:
        return analysis
    result = json.loads(json.dumps(analysis or {}, default=str))
    parcel = parcel_details.get("parcel") or {}
    area = _usable_number(parcel.get("parcel_extent_m2")) or 0.0

    land_total = _merge_anchor_range(result.get("expert_land_total_eur"), profile["land_total_eur"])
    result["expert_land_total_eur"] = land_total
    properties = result.get("property_analyses") or {}
    land = properties.get("land") or {}
    if area > 0:
        land["sale_price_per_m2"] = {
            "low": round(land_total["low"] / area, 2),
            "high": round(land_total["high"] / area, 2),
        }
    land["evidence_status"] = "expert_calibrated_exact_micro_market"
    land["confidence"] = "high"
    land["market_area_used"] = profile["profile_id"]
    properties["land"] = land
    result["property_analyses"] = properties

    result["detached_house_total_build_cost_eur"] = _merge_anchor_range(
        result.get("detached_house_total_build_cost_eur"),
        profile["single_villa_total_build_cost_eur"],
    )
    result["detached_house_total_sale_eur"] = _merge_anchor_range(
        result.get("detached_house_total_sale_eur"),
        profile["single_villa_completed_sale_eur"],
    )
    result["detached_house_total_rent_eur_month"] = _merge_anchor_range(
        result.get("detached_house_total_rent_eur_month"),
        profile["single_villa_monthly_rent_eur"],
    )
    result["market_tier"] = profile["market_tier"]
    result["expert_micro_market_profile"] = {
        "profile_id": profile["profile_id"],
        "basis": profile["basis"],
        "status": "applied_as_minimum_consistency_anchor",
    }
    result["evidence_status"] = "expert_calibrated_exact_micro_market"
    return result


def _fallback_market_analysis(parcel_details: dict[str, Any]) -> dict[str, Any]:
    parcel = parcel_details.get("parcel") or {}
    district = _market_district_key(parcel.get("district"))
    defaults = {
        "limassol": {"land": (450, 1050), "apartment": (3100, 5000, 16, 25), "house": (2600, 4300, 12, 20), "office": (2300, 3900, 15, 24), "shop": (2600, 4500, 17, 28), "construction": (1650, 2250)},
        "nicosia": {"land": (300, 750), "apartment": (2200, 3400, 10, 16), "house": (1900, 3100, 8, 13), "office": (1900, 3200, 11, 18), "shop": (2100, 3500, 12, 20), "construction": (1550, 2100)},
        "larnaca": {"land": (280, 720), "apartment": (2400, 3800, 12, 19), "house": (2100, 3400, 9, 15), "office": (1800, 3000, 10, 16), "shop": (2100, 3400, 12, 19), "construction": (1550, 2150)},
        "paphos": {"land": (320, 850), "apartment": (2700, 4300, 13, 21), "house": (2400, 3900, 11, 18), "office": (1800, 3000, 10, 16), "shop": (2100, 3500, 12, 20), "construction": (1600, 2200)},
        "famagusta": {"land": (250, 720), "apartment": (2500, 4100, 12, 21), "house": (2200, 3700, 10, 18), "office": (1700, 2800, 9, 15), "shop": (2000, 3400, 11, 19), "construction": (1550, 2150)},
        "cyprus": {"land": (250, 750), "apartment": (2300, 3800, 11, 19), "house": (2000, 3400, 9, 16), "office": (1800, 3000, 10, 17), "shop": (2100, 3500, 12, 20), "construction": (1550, 2150)},
    }[district]

    def analysis(kind: str) -> dict[str, Any]:
        if kind == "land":
            low, high = defaults["land"]
            return {"property_type": "land", "evidence_status": "district_fallback_estimate", "relevant_observation_count": 0, "sale_price_per_m2": {"low": low, "high": high}, "rent_per_m2_month": None, "top_comparables": [], "warnings": []}
        sale_low, sale_high, rent_low, rent_high = defaults[kind]
        return {"property_type": kind, "evidence_status": "district_fallback_estimate", "relevant_observation_count": 0, "sale_price_per_m2": {"low": sale_low, "high": sale_high}, "rent_per_m2_month": {"low": rent_low, "high": rent_high}, "top_comparables": [], "warnings": []}

    properties = {kind: analysis(kind) for kind in ("apartment", "house", "land", "office", "shop")}
    parcel_area = _usable_number(parcel.get("parcel_extent_m2")) or 0.0
    generic_low, generic_high = defaults["construction"]
    fallback_external_low = min(max(120_000.0, parcel_area * 120.0), 650_000.0)
    fallback_external_high = min(max(220_000.0, parcel_area * 240.0), 1_250_000.0)
    return {
        **properties["apartment"],
        "property_analyses": properties,
        "construction_cost_eur_m2": {"low": generic_low, "high": generic_high},
        "construction_cost_by_type": {
            "apartment": {"low": generic_low, "high": generic_high},
            "house": {"low": round(generic_low * 1.20), "high": round(generic_high * 1.45)},
            "office": {"low": round(generic_low * 1.05), "high": round(generic_high * 1.20)},
            "shop": {"low": round(generic_low * 1.05), "high": round(generic_high * 1.20)},
        },
        "detached_house_external_works_eur": {"low": round(fallback_external_low), "high": round(fallback_external_high)},
        "detached_house_total_build_cost_eur": None,
        "detached_house_total_sale_eur": None,
        "detached_house_total_rent_eur_month": None,
        "expert_land_total_eur": None,
        "land_asking_to_achievable_discount_percent": LAND_ASKING_DISCOUNT_PERCENT,
        "market_tier": "district_fallback",
        "market_area_used": parcel.get("municipality") or parcel.get("district") or "Cyprus",
        "evidence_status": "district_fallback_estimate",
        "source_registry": source_status_summary(),
        "official_data_context": {"status": "fallback"},
        "cached": False,
    }


def _normalise_live_range(low: Any, high: Any, fallback_low: float, fallback_high: float) -> dict[str, float]:
    low_value = _usable_number(low) or fallback_low
    high_value = _usable_number(high) or fallback_high
    return {"low": round(min(low_value, high_value), 2), "high": round(max(low_value, high_value), 2)}


MARKET_RANGE_CALIBRATION_VERSION = "central-band-v3-prime-villa"


def _calibrated_numeric_range(
    value: dict[str, Any] | None,
    *,
    max_half_width_percent: float,
) -> dict[str, float] | None:
    """Keep the evidence centre while trimming extreme outer tails."""
    if not isinstance(value, dict):
        return None
    low = _usable_number(value.get("low"))
    high = _usable_number(value.get("high"))
    ...
    max_half = centre * max_half_width_percent / 100.0
    calibrated_low = max(low, centre - max_half)
    calibrated_high = min(high, centre + max_half)
    return {"low": round(calibrated_low, 2), "high": round(max(calibrated_high, calibrated_low), 2)}


def _market_range_width_percent(analysis: dict[str, Any]) -> float:
    evidence_status = str(analysis.get("evidence_status") or "").lower()
    confidence = str(analysis.get("confidence") or "low").lower()
    count = int(analysis.get("relevant_observation_count") or 0)
    if confidence == "high" and count >= 20:
        return 7.0
    if confidence in {"high", "medium"} and count >= 8:
        return 8.0
    if evidence_status == "targeted_live_web_research":
        return 10.0
    if count > 0:
        return 12.0
    return 15.0
...
def _calibrate_market_analysis_ranges(value: dict[str, Any]) -> dict[str, Any]:
    """Create tighter screening bands while retaining raw evidence ranges."""
    result = json.loads(json.dumps(value or {}, default=str))
    properties = result.get("property_analyses") or {}
    for kind, analysis in list(properties.items()):
        if not isinstance(analysis, dict):
            continue
        width = _market_range_width_percent(analysis)
        sale = analysis.get("sale_price_per_m2")
        rent = analysis.get("rent_per_m2_month")
        if isinstance(sale, dict):
            analysis.setdefault("raw_sale_price_per_m2", sale)
            analysis["sale_price_per_m2"] = _calibrated_numeric_range(sale, max_half_width_percent=width)
        if isinstance(rent, dict):
            analysis.setdefault("raw_rent_per_m2_month", rent)
            analysis["rent_per_m2_month"] = _calibrated_numeric_range(rent, max_half_width_percent=width)
        analysis["range_calibration"] = {
            "version": MARKET_RANGE_CALIBRATION_VERSION,
            "max_half_width_percent": width,
            "purpose": "screening_not_valuation",
        }
        properties[kind] = analysis
    result["property_analyses"] = properties
    if properties.get("apartment"):
        result.update(properties["apartment"])
    construction = result.get("construction_cost_eur_m2")
    if isinstance(construction, dict):
        result.setdefault("raw_construction_cost_eur_m2", construction)
        result["construction_cost_eur_m2"] = _calibrated_numeric_range(
            construction,
            max_half_width_percent=8.0,
        )
    result["range_calibration_version"] = MARKET_RANGE_CALIBRATION_VERSION
    return result


def _market_analysis_needs_live_research(
    parcel_details: dict[str, Any],
    observed: dict[str, Any],
) -> bool:
    """Live web research is the primary source whenever it is enabled.

    Stored observations remain useful as supporting comparables and as a fallback
    if the online research call fails, but they no longer suppress online research.
    """
    return LIVE_MARKET_ENABLED


def _apply_live_research(base: dict[str, Any], live: LiveMarketResearchResult | None) -> dict[str, Any]:
    if not live:
        return base
    result = json.loads(json.dumps(base, default=str))
    properties = result.get("property_analyses") or {}
    mapping = {"apartment": live.apartment, "house": live.house, "office": live.office, "shop": live.shop}
    for kind, band in mapping.items():
        existing = properties.get(kind) or {}
        sale_fallback = existing.get("sale_price_per_m2") or {"low": 1, "high": 1}
        rent_fallback = existing.get("rent_per_m2_month") or {"low": 1, "high": 1}
        existing["sale_price_per_m2"] = _normalise_live_range(
            band.sale_low_eur_m2, band.sale_high_eur_m2,
            sale_fallback["low"], sale_fallback["high"],
        )
        existing["rent_per_m2_month"] = _normalise_live_range(
            band.rent_low_eur_m2_month, band.rent_high_eur_m2_month,
            rent_fallback["low"], rent_fallback["high"],
        )
        existing["evidence_status"] = "targeted_live_web_research"
        properties[kind] = existing

    land = properties.get("land") or {}
    land_fallback = land.get("sale_price_per_m2") or {"low": 1, "high": 1}
    land["sale_price_per_m2"] = _normalise_live_range(
        live.land_sale_low_eur_m2, live.land_sale_high_eur_m2,
        land_fallback["low"], land_fallback["high"],
    )
    land["evidence_status"] = "targeted_live_web_research"
    properties["land"] = land

    result["property_analyses"] = properties
    result.update(properties["apartment"])
    construction = result.get("construction_cost_eur_m2") or {
        "low": SCREENING_COST_LOW_EUR_M2, "high": SCREENING_COST_HIGH_EUR_M2,
    }
    generic_construction = _normalise_live_range(
        live.construction_cost_low_eur_m2,
        live.construction_cost_high_eur_m2,
        construction["low"], construction["high"],
    )
    house_construction = _normalise_live_range(
        live.detached_house_construction_cost_low_eur_m2,
        live.detached_house_construction_cost_high_eur_m2,
        generic_construction["low"] * 1.20,
        generic_construction["high"] * 1.45,
    )
    result["construction_cost_eur_m2"] = generic_construction
    result["construction_cost_by_type"] = {
        "apartment": generic_construction,
        "house": house_construction,
        "office": {"low": round(generic_construction["low"] * 1.05, 2), "high": round(generic_construction["high"] * 1.20, 2)},
        "shop": {"low": round(generic_construction["low"] * 1.05, 2), "high": round(generic_construction["high"] * 1.20, 2)},
    }
    result["detached_house_external_works_eur"] = _normalise_live_range(
        live.detached_house_external_works_low_eur,
        live.detached_house_external_works_high_eur,
        120_000.0, 250_000.0,
    )
    result["detached_house_total_build_cost_eur"] = _normalise_live_range(
        live.detached_house_total_build_cost_low_eur,
        live.detached_house_total_build_cost_high_eur,
        1.0, 1.0,
    )
    result["detached_house_total_sale_eur"] = _normalise_live_range(
        live.detached_house_total_sale_low_eur,
        live.detached_house_total_sale_high_eur,
        1.0, 1.0,
    )
    result["detached_house_total_rent_eur_month"] = _normalise_live_range(
        live.detached_house_total_rent_low_eur_month,
        live.detached_house_total_rent_high_eur_month,
        1.0, 1.0,
    )
    result["land_asking_to_achievable_discount_percent"] = round(
        min(max(float(live.land_asking_to_achievable_discount_percent), 0.0), 30.0), 2
    )
    result["market_tier"] = str(live.market_tier or "unclassified")
    result["market_area_used"] = live.market_area_used
    result["evidence_date"] = live.evidence_date
    result["evidence_status"] = "targeted_live_web_research"
    return result


def _run_live_market_research(parcel_details: dict[str, Any]) -> LiveMarketResearchResult | None:
    """One paid model call: web research and strict structuring happen together."""
    ...
    market_context = parcel_details.get("_market_context") or {}
    zones = ", ".join(
        str(item.get("zone")) for item in parcel_details.get("planning_zones") or [] if item.get("zone")
    )
    nearby_roads = ", ".join(str(value) for value in market_context.get("nearby_roads") or [] if value)
    exact_address = market_context.get("display_name") or market_context.get("formatted_address")
    prompt = f"""
Research the current property market around this exact Cyprus parcel and return the required structured fields.

SUBJECT SITE
- parcel id: {parcel.get('parcel_id')}
- parcel number: {parcel.get('parcel_number')}
- exact coordinates: {market_context.get('centroid_lat')}, {market_context.get('centroid_lon')}
- reverse-geocoded address: {exact_address}
- nearest/adjacent roads: {nearby_roads}
- quarter/locality: {parcel.get('quarter')}
- municipality: {parcel.get('municipality')}
- district: {parcel.get('district')}
- planning zone(s): {zones}
- parcel area: {parcel.get('parcel_extent_m2')} m²
- theoretical density-counted floor area: {potential.get('theoretical_max_floor_area_m2')} m²
- maximum floors: {max([_usable_number(z.get('max_floors')) or 0 for z in parcel_details.get('planning_zones') or []] or [0])}

RESEARCH PRIORITY
1. The named adjacent roads, the exact quarter and properties immediately around the coordinates.
2. Comparable evidence within roughly 500 metres.
3. The same neighbourhood within roughly 1.5 kilometres.
4. The municipality only when micro-market evidence is genuinely thin.
5. District-wide figures only as an explicitly weak fallback.

VALUATION RULES
- Prices must exclude VAT so PLANA does not mix VAT-inclusive and VAT-exclusive figures.
- Land figures must reflect current public ASKING evidence for comparable bare development land in the subject micro-market.
- Do not substitute ordinary Germasogeia, Limassol or district averages for a prime hillside, seafront, view or luxury enclave.
- Exclude sites sold with completed buildings, permits or clearly different planning rights unless the subject shares those characteristics.
- Provide a realistic asking-to-achievable discount percentage for land.
- House €/m² must reflect new-build homes in the same micro-market and market tier, not the district median.
- Estimate the total achievable sale range for ONE newly completed detached house developed on a plot comparable to this subject site.
- Also estimate the TOTAL hard build cost for that complete house, including all constructed areas and external works but excluding land, VAT, professional fees, finance and sales costs.
- Also estimate the TOTAL achievable monthly rent for that complete house; do not derive it from ordinary district rent €/m².
- Include the location, plot, view, pool, landscaping and luxury-market premium in all whole-property totals.
- Do not discard a verified adjacent €10m+ house or €35k–€45k monthly villa merely because district averages are lower.
- Generic construction €/m² is for apartment, office and shop hard construction.
- Detached-house construction €/m² is for the relevant market tier and includes high-spec finishes and MEP on TOTAL constructed area, but excludes land, VAT, professional/finance/sales costs and external works.
- External-works totals must separately cover likely pool, landscaping, driveways, boundaries, services and site works.
- The detached-house TOTAL build-cost field must already include those external works so it can be used as the primary single-villa cost.
- Use multiple current public sources where possible. Do not use one extreme listing as the whole range.
- All low/high pairs must be ordered, non-zero and commercially realistic as of the evidence date.
"""
    tool: dict[str, Any] = {"type": "web_search", "search_context_size": "medium"}
    city = parcel.get("municipality") or parcel.get("quarter")
    region = parcel.get("district")
    tool["user_location"] = {
        "type": "approximate",
        "country": "CY",
        ...
        input=[
            {
                "role": "system",
                "content": (
                    "You are an experienced Cyprus development valuer. Use exact micro-market web evidence, "
                    "distinguish ordinary from luxury locations, and return only the requested structured result. "
                    "Never make land, construction and completed-sale figures internally inconsistent."
                ),
            },
            {"role": "user", "content": prompt.strip()},
        ],
        text_format=LiveMarketResearchResult,
    )
    return response.output_parsed

async def _resolve_market_location_context(
    centroid_lat: float | None,
    centroid_lon: float | None,
) -> dict[str, Any]:
    """Resolve road-level context without spending AI tokens."""
    ...
    async def nearby_road_lookup() -> list[str]:
        query = f"""
[out:json][timeout:8];
way(around:260,{centroid_lat},{centroid_lon})["highway"]["name"];
out tags 80;
""".strip()
        try:
            response = await client.post(OVERPASS_API, content=query, timeout=10.0)
            if response.status_code != 200:
                return []
            names: list[str] = []
            for item in (response.json() or {}).get("elements") or []:
                tags = item.get("tags") or {}
    ...
    reverse, roads = await asyncio.gather(reverse_lookup(), nearby_road_lookup())
    context.update(reverse)
    combined = []
    if reverse.get("road"):
        combined.append(reverse["road"])
    combined.extend(roads)
    deduped: list[str] = []
    for value in combined:
        text = clean_text(value)
        if text and text.casefold() not in {item.casefold() for item in deduped}:
            deduped.append(text)
    context["nearby_roads"] = deduped[:8]
    return context


def _land_market_needs_validation(
    analysis: dict[str, Any],
    parcel_details: dict[str, Any],
) -> bool:
    """Trigger a focused second search only for suspicious high-value sites."""
    parcel = parcel_details.get("parcel") or {}
    parcel_area = _usable_number(parcel.get("parcel_extent_m2"))
    land_sale = (((analysis.get("property_analyses") or {}).get("land") or {}).get("sale_price_per_m2") or {})
    land_low = _usable_number(land_sale.get("low"))
    land_high = _usable_number(land_sale.get("high"))
    if parcel_area is None or land_low is None or land_high is None:
        return True

    direct_land_mid = parcel_area * (land_low + land_high) / 2.0
    completed_house = analysis.get("detached_house_total_sale_eur") or {}
    house_low = _usable_number(completed_house.get("low"))
    house_high = _usable_number(completed_house.get("high"))
    house_mid = ((house_low or 0.0) + (house_high or 0.0)) / 2.0 if house_low and house_high else 0.0
    tier = str(analysis.get("market_tier") or "").casefold()
    dls_value = _usable_number(parcel.get("price_2021"))

    locality = " ".join(
        str(parcel.get(key) or "")
        for key in ("quarter", "municipality", "district")
    ).casefold()
    premium_locality = any(token in locality for token in (
    ...
    luxury_site = any(token in tier for token in ("luxury", "prime", "ultra"))
    inconsistent_with_house = house_mid >= 2_500_000 and direct_land_mid < house_mid * 0.30
    inconsistent_with_dls = dls_value is not None and direct_land_mid < dls_value * 0.70
    abnormally_low_prime_land = luxury_site and direct_land_mid < 1_500_000
    premium_locality_miss = premium_locality and parcel_area >= 700 and direct_land_mid < 1_500_000
    return (
        inconsistent_with_house
        or inconsistent_with_dls
        or abnormally_low_prime_land
        or premium_locality_miss
    )


def _run_live_land_research(parcel_details: dict[str, Any]) -> LiveLandResearchResult | None:
    """Focused one-call land validation, used only when the first pass looks implausible."""
    if not LIVE_MARKET_ENABLED:
        return None
    client = state.get("openai")
    if not client:
    ...
    roads = ", ".join(str(value) for value in context.get("nearby_roads") or [] if value)
    prompt = f"""
Validate only the current asking value of BARE DEVELOPMENT LAND for this exact Cyprus parcel.

SUBJECT
- exact coordinates: {context.get('centroid_lat')}, {context.get('centroid_lon')}
- address: {context.get('display_name')}
- adjacent roads: {roads}
- quarter/locality: {parcel.get('quarter')}
- municipality: {parcel.get('municipality')}
- district: {parcel.get('district')}
- zone(s): {zones}
- plot area: {parcel.get('parcel_extent_m2')} m²

Search current public listings in this order:
1. Same road or adjoining roads.
2. Within roughly 500 metres.
3. Same neighbourhood within roughly 1.5 kilometres.
4. Same municipality only if necessary.

Use comparable bare residential/development plots with similar location, planning rights, access and plot size.
Do not use broad district averages when the exact micro-market is premium.
Do not mix completed houses with bare land.
Do not automatically reject expensive nearby land as an outlier when several luxury properties establish that micro-market.
Return a realistic CURRENT ASKING-price range in EUR per plot m², before PLANA's separate 10% achievable-price calibration.
Return the number of genuinely relevant comparables and a confidence label.
"""
    tool: dict[str, Any] = {"type": "web_search", "search_context_size": "medium"}
    city = parcel.get("municipality") or parcel.get("quarter")
    region = parcel.get("district")
        ...
        input=[
            {
                "role": "system",
                "content": (
                    "You are a Cyprus land valuer performing a focused comparable-land check. "
                    "Use exact micro-market evidence and return only the requested structured result."
                ),
            },
            {"role": "user", "content": prompt.strip()},
        ],
        text_format=LiveLandResearchResult,
    )
    return response.output_parsed


def _apply_focused_land_research(
    base: dict[str, Any],
    focused: LiveLandResearchResult | None,
) -> dict[str, Any]:
    if not focused:
        return base
    result = json.loads(json.dumps(base, default=str))
    properties = result.get("property_analyses") or {}
    land = properties.get("land") or {}
    current = land.get("sale_price_per_m2") or {"low": 1.0, "high": 1.0}
    land["initial_live_sale_price_per_m2"] = current
    land["sale_price_per_m2"] = _normalise_live_range(
        focused.land_sale_low_eur_m2,
        focused.land_sale_high_eur_m2,
        current.get("low") or 1.0,
        current.get("high") or 1.0,
    )
    land["evidence_status"] = "focused_exact_micro_market_land_research"
    land["confidence"] = str(focused.confidence or "low").strip().lower()
    land["relevant_observation_count"] = int(focused.comparable_count or 0)
    land["market_area_used"] = focused.market_area_used
    land["evidence_date"] = focused.evidence_date
    properties["land"] = land
    result["property_analyses"] = properties
    result["land_validation_used"] = True
    result["land_validation_confidence"] = land["confidence"]
    result["land_validation_comparable_count"] = land["relevant_observation_count"]
    return result


def _luxury_villa_research_needed(analysis: dict[str, Any], parcel_details: dict[str, Any]) -> bool:
    parcel = parcel_details.get("parcel") or {}
    locality = _normalised_market_text(parcel.get("quarter"), parcel.get("municipality"), parcel.get("district"))
    context = parcel_details.get("_market_context") or {}
    context_text = _normalised_market_text(context.get("display_name"), *(context.get("nearby_roads") or []))
    tier = _normalised_market_text(analysis.get("market_tier"))
    premium_tokens = (
        "potamos germasogeias", "amathounta", "agios tychonas", "mouttagiaka",
        "parekklisia", "aphrodite hills", "coral bay", "protaras", "ayia napa",
    )
    premium = any(token in locality or token in context_text for token in premium_tokens) or any(
        token in tier for token in ("luxury", "prime", "ultra")
    )
    if not premium:
        return False
    sale = analysis.get("detached_house_total_sale_eur") or {}
    build = analysis.get("detached_house_total_build_cost_eur") or {}
    rent = analysis.get("detached_house_total_rent_eur_month") or {}
    sale_mid = ((_usable_number(sale.get("low")) or 0.0) + (_usable_number(sale.get("high")) or 0.0)) / 2.0
    build_mid = ((_usable_number(build.get("low")) or 0.0) + (_usable_number(build.get("high")) or 0.0)) / 2.0
    rent_mid = ((_usable_number(rent.get("low")) or 0.0) + (_usable_number(rent.get("high")) or 0.0)) / 2.0
    return sale_mid < 4_000_000 or build_mid < 2_000_000 or rent_mid < 15_000


def _run_live_luxury_villa_research(parcel_details: dict[str, Any]) -> LiveLuxuryVillaResearchResult | None:
    """One focused call for prime single-villa whole-property economics."""
    if not LIVE_MARKET_ENABLED:
        return None
    client = state.get("openai")
    if not client:
    ...
    context = parcel_details.get("_market_context") or {}
    roads = ", ".join(str(value) for value in context.get("nearby_roads") or [] if value)
    zones = ", ".join(str(item.get("zone")) for item in parcel_details.get("planning_zones") or [] if item.get("zone"))
    prompt = f"""
Perform a focused PRIME/LUXURY SINGLE-VILLA market check for this exact Cyprus parcel.

SUBJECT
- coordinates: {context.get('centroid_lat')}, {context.get('centroid_lon')}
- address: {context.get('display_name')}
- adjacent roads: {roads}
- locality: {parcel.get('quarter')} / {parcel.get('municipality')} / {parcel.get('district')}
- zone(s): {zones}
- plot area: {parcel.get('parcel_extent_m2')} m²
- density-counted area: {potential.get('theoretical_max_floor_area_m2')} m²

Return WHOLE-PROPERTY totals, not ordinary district €/m² averages:
1. Current market value of the bare plot.
2. Total hard construction and external-works cost for one newly built high-spec villa on this site, excluding land, VAT, professional fees, finance and selling costs. Include basements, garages, plant, covered outdoor areas, pool, landscaping, retaining/boundary works, driveways and high-spec MEP/finishes.
3. Achievable completed sale value of the entire new villa.
4. Achievable long-term monthly rent of the entire new villa.

Search the same road and immediately adjacent roads first, then within 500 m, then the exact neighbourhood.
A verified adjacent €10m+ new villa and €35k–€45k/month luxury rental are relevant evidence and must not be replaced by ordinary Germasogeia averages.
Exclude VAT from sale and land figures. Use multiple sources where available and return realistic ordered ranges.
"""
    tool: dict[str, Any] = {"type": "web_search", "search_context_size": "medium"}
    city = parcel.get("municipality") or parcel.get("quarter")
    region = parcel.get("district")
    tool["user_location"] = {
        "type": "approximate", "country": "CY",
        **({"city": str(city)} if city else {}),
        **({"region": str(region)} if region else {}),
    }
    response = client.responses.parse(
        ...
        include=["web_search_call.action.sources"],
        input=[
            {"role": "system", "content": "You are a senior Cyprus luxury-residential valuer and development cost consultant. Return only the requested structured totals and do not substitute district averages for the exact micro-market."},
            {"role": "user", "content": prompt.strip()},
        ],
        text_format=LiveLuxuryVillaResearchResult,
    )
    return response.output_parsed


def _apply_luxury_villa_research(
    base: dict[str, Any],
    focused: LiveLuxuryVillaResearchResult | None,
    parcel_details: dict[str, Any],
) -> dict[str, Any]:
    if not focused:
        return base
    result = json.loads(json.dumps(base or {}, default=str))
    land_total = _normalise_live_range(focused.land_total_low_eur, focused.land_total_high_eur, 1.0, 1.0)
    result["expert_land_total_eur"] = land_total
    area = _usable_number((parcel_details.get("parcel") or {}).get("parcel_extent_m2")) or 0.0
    properties = result.get("property_analyses") or {}
    land = properties.get("land") or {}
    if area > 0:
        land["sale_price_per_m2"] = {"low": round(land_total["low"] / area, 2), "high": round(land_total["high"] / area, 2)}
    land["evidence_status"] = "focused_luxury_villa_micro_market"
    land["confidence"] = str(focused.confidence or "low").strip().lower()
    land["relevant_observation_count"] = int(focused.comparable_land_count or 0)
    properties["land"] = land
    result["property_analyses"] = properties
    result["detached_house_total_build_cost_eur"] = _normalise_live_range(
        focused.total_build_cost_low_eur, focused.total_build_cost_high_eur, 1.0, 1.0
    )
    result["detached_house_total_sale_eur"] = _normalise_live_range(
        focused.completed_sale_low_eur, focused.completed_sale_high_eur, 1.0, 1.0
    )
    result["detached_house_total_rent_eur_month"] = _normalise_live_range(
        focused.monthly_rent_low_eur, focused.monthly_rent_high_eur, 1.0, 1.0
    )
    result["market_tier"] = str(focused.market_tier or "prime_luxury")
    result["market_area_used"] = focused.market_area_used
    result["evidence_date"] = focused.evidence_date
    result["luxury_villa_research"] = {
        "status": "focused_whole_property_totals",
        "confidence": str(focused.confidence or "low").strip().lower(),
        "sale_comparable_count": int(focused.comparable_sale_count or 0),
        "rent_comparable_count": int(focused.comparable_rent_count or 0),
    }
    return result


def _reconcile_prime_villa_consistency(analysis: dict[str, Any]) -> dict[str, Any]:
    """Prevent internally impossible prime-villa outputs without changing ordinary sites."""
    result = json.loads(json.dumps(analysis or {}, default=str))
    tier = _normalised_market_text(result.get("market_tier"))
    if not any(token in tier for token in ("luxury", "prime", "ultra")):
        return result
    sale = result.get("detached_house_total_sale_eur") or {}
    sale_low = _usable_number(sale.get("low"))
    sale_high = _usable_number(sale.get("high"))
    if sale_low is None or sale_high is None:
        return result
    sale_low, sale_high = min(sale_low, sale_high), max(sale_low, sale_high)
    # Construction below roughly 25% of prime completed value is usually missing
    # basements, external works or luxury specification.
    build_anchor = {"low": sale_low * 0.28, "high": sale_high * 0.38}
    result["detached_house_total_build_cost_eur"] = _merge_anchor_range(
        result.get("detached_house_total_build_cost_eur"), build_anchor, replace_below_ratio=0.80
    )
    # Prime long-term rent is cross-checked against a 3.5%–5.0% gross annual yield.
    rent_anchor = {"low": sale_low * 0.035 / 12.0, "high": sale_high * 0.050 / 12.0}
    result["detached_house_total_rent_eur_month"] = _merge_anchor_range(
        result.get("detached_house_total_rent_eur_month"), rent_anchor, replace_below_ratio=0.75
    )
    return result


def _persistent_market_cache_get(cache_key: str) -> dict[str, Any] | None:
    """Read an app-generated market estimate cache row when the optional table exists."""
    supabase = state.get("supabase")
    if not supabase:
        return None
    try:
        response = (
            supabase.table(MARKET_ESTIMATE_CACHE_TABLE)
        ...
        # The cache table is optional. Absence must never block an analysis.
        return None


def _persistent_market_cache_put(cache_key: str, payload: dict[str, Any]) -> None:
    """Persist only PLANA's derived ranges, never copied listing descriptions or images."""
    supabase = state.get("supabase")
    if not supabase:
        return
    try:
        ...
        expires = datetime.fromtimestamp(
            now_dt.timestamp() + MARKET_ANALYSIS_CACHE_TTL_SECONDS,
            tz=timezone.utc,
        )
        supabase.table(MARKET_ESTIMATE_CACHE_TABLE).upsert({
            "cache_key": cache_key,
            "payload": payload,
...
async def build_hybrid_market_analysis(
    parcel_details: dict[str, Any], *, centroid_lat: float | None = None, centroid_lon: float | None = None,
) -> dict[str, Any]:
    parcel = parcel_details.get("parcel") or {}
    potential = parcel_details.get("development_potential") or {}
    zone_codes = sorted({
        _registry_text(item.get("zone"))
        for item in parcel_details.get("planning_zones") or []
        if item.get("zone")
    })
    cache_key = json.dumps({
        ...
        "parcel_area_m2": _usable_number(parcel.get("parcel_extent_m2")),
        "buildable_area_m2": _usable_number(potential.get("theoretical_max_floor_area_m2")),
        "zone_codes": zone_codes,
        "centroid_lat": round(centroid_lat, 5) if centroid_lat is not None else None,
        "centroid_lon": round(centroid_lon, 5) if centroid_lon is not None else None,
        "hybrid": 9,
    }, sort_keys=True, default=str)
    cached = MARKET_ANALYSIS_CACHE.get(cache_key)
    if cached and time.time() - cached[0] < MARKET_ANALYSIS_CACHE_TTL_SECONDS:
        return {**cached[1], "cached": True, "cache_source": "memory"}
    persistent = await asyncio.to_thread(_persistent_market_cache_get, cache_key)
    if persistent:
        MARKET_ANALYSIS_CACHE[cache_key] = (time.time(), persistent)
        return {**persistent, "cached": True, "cache_source": "supabase"}

    fallback = _fallback_market_analysis(parcel_details)
    ...
    merged = json.loads(json.dumps(fallback, default=str))
    existing_properties = supabase_result.get("property_analyses") or {}
    for kind, existing in existing_properties.items():
        if not isinstance(existing, dict):
            continue
        target = merged["property_analyses"].setdefault(kind, {})
        if (existing.get("sale_price_per_m2") or {}).get("low") is not None:
            target["sale_price_per_m2"] = existing.get("sale_price_per_m2")
        if (existing.get("rent_per_m2_month") or {}).get("low") is not None:
            target["rent_per_m2_month"] = existing.get("rent_per_m2_month")
        target["relevant_observation_count"] = existing.get("relevant_observation_count") or 0
        target["evidence_status"] = existing.get("evidence_status") or target.get("evidence_status")
        if existing.get("top_comparables"):
            target["top_comparables"] = existing.get("top_comparables")
    merged.update(merged["property_analyses"]["apartment"])
    merged["supabase_market_status"] = supabase_result.get("evidence_status")

    market_context = await _resolve_market_location_context(centroid_lat, centroid_lon)
    research_details = json.loads(json.dumps(parcel_details, default=str))
    research_details["_market_context"] = market_context

    # Online-first market policy: one broad structured pass is used for most
    # plots. A second, focused land-only call is made only when the first result
    # is internally inconsistent with a high-value house micro-market or DLS context.
    if LIVE_MARKET_ENABLED:
        try:
            live = await asyncio.wait_for(
                asyncio.to_thread(_run_live_market_research, research_details),
                timeout=MARKET_RESEARCH_TIMEOUT_SECONDS,
            )
            if live:
                merged = _apply_live_research(merged, live)
                merged["market_source_priority"] = "live_web_primary_database_support"
                merged["database_comparables_role"] = "supporting_evidence"
                merged["market_location_context"] = market_context

                if _land_market_needs_validation(merged, parcel_details):
                    try:
                        focused_land = await asyncio.wait_for(
                            asyncio.to_thread(_run_live_land_research, research_details),
                            timeout=MARKET_RESEARCH_TIMEOUT_SECONDS,
                        )
                        if focused_land:
                            merged = _apply_focused_land_research(merged, focused_land)
                            merged["land_research_mode"] = "focused_second_pass"
                        else:
                            merged["land_research_mode"] = "primary_pass_only_validation_empty"
                    except Exception as land_exc:
                        print(f"PLANA focused land validation fallback: {land_exc!r}", file=sys.stderr)
                        merged["land_research_mode"] = "primary_pass_only_validation_failed"
                        merged["land_validation_error"] = str(land_exc)
                else:
                    merged["land_research_mode"] = "primary_pass_only_consistent"

                # Prime single-villa sites require whole-property totals. This
                # extra call runs only when the broad pass is clearly inconsistent.
                research_details["_market_context"] = market_context
                if _luxury_villa_research_needed(merged, research_details):
                    try:
                        focused_villa = await asyncio.wait_for(
                            asyncio.to_thread(_run_live_luxury_villa_research, research_details),
                            timeout=MARKET_RESEARCH_TIMEOUT_SECONDS,
                        )
                        if focused_villa:
                            merged = _apply_luxury_villa_research(merged, focused_villa, research_details)
                    except Exception as villa_exc:
                        print(f"PLANA focused luxury-villa validation fallback: {villa_exc!r}", file=sys.stderr)
                        merged["luxury_villa_research_error"] = str(villa_exc)
            else:
                merged["market_source_priority"] = "database_fallback"
                merged["live_research_error"] = "Online market research returned no structured result."
        except Exception as exc:
            print(f"PLANA online-first market research fallback: {exc!r}", file=sys.stderr)
            merged["market_source_priority"] = "database_fallback"
            merged["database_comparables_role"] = "primary_only_because_live_failed"
            merged["live_research_error"] = str(exc)
    else:
        merged["market_source_priority"] = "database_primary_live_disabled"
        merged["live_research_skipped"] = "disabled"

    # Expert micro-market anchors are deterministic and also protect the result
    # when web search is unavailable, sparse or structurally misclassified.
    merged = _apply_expert_micro_market_profile(merged, research_details)
    merged = _reconcile_prime_villa_consistency(merged)
    merged = _calibrate_market_analysis_ranges(merged)
    MARKET_ANALYSIS_CACHE[cache_key] = (time.time(), merged)
    await asyncio.to_thread(_persistent_market_cache_put, cache_key, merged)
    return merged
...
async def parcel_market_analysis(payload: ParcelMarketRequest) -> dict[str, Any]:
    parcel_details = await get_canonical_parcel_details(payload.parcel_id)
    return await build_hybrid_market_analysis(
        parcel_details,
...
async def parcel_opportunity_analysis(payload: ParcelOpportunityRequest) -> dict[str, Any]:
    ...
    parcel_details = await get_canonical_parcel_details(payload.parcel_id)
    market_analysis = await build_hybrid_market_analysis(
        parcel_details,
        centroid_lat=payload.centroid_lat,
        centroid_lon=payload.centroid_lon,
    )
...
def _screening_pair(
    ...
    *,
    ...
    if centre == 0:
        return low, high
    max_half = abs(centre) * max_half_width_percent / 100.0
    return max(low, centre - max_half), min(high, centre + max_half)


def _zone_option_fallback(parcel_details: dict[str, Any]) -> list[dict[str, Any]]:
    """Use the verified registry matrix, with a conservative family fallback."""
    combined: dict[str, str] = {}
    for zone in parcel_details.get("planning_zones") or []:
        matrix = zone.get("allowed_uses") or _zone_use_matrix(str(zone.get("zone") or ""), zone.get("description") or zone.get("remarks"))
        for use_type, status in matrix.items():
            if status == "not_supported":
                continue
            if combined.get(use_type) != "allowed":
                combined[use_type] = status
    if not combined:
        combined = {"house": "conditional", "apartments": "conditional"}
    return [{"type": use_type, "status": status, "reason": "Verified planning-zone use matrix."} for use_type, status in combined.items()]
...
def build_viable_development_options(
    ...
    market_root = _calibrate_market_analysis_ranges(
        opportunity.get("market_analysis") or _fallback_market_analysis(parcel_details)
    )
    property_analyses = market_root.get("property_analyses") or {"apartment": market_root}
    construction_range = market_root.get("construction_cost_eur_m2") or {}
    cost_low = _usable_number(construction_range.get("low")) or SCREENING_COST_LOW_EUR_M2
    cost_high = _usable_number(construction_range.get("high")) or SCREENING_COST_HIGH_EUR_M2
    cost_low, cost_high = min(cost_low, cost_high), max(cost_low, cost_high)
    construction_by_type = market_root.get("construction_cost_by_type") or {}
    detached_house_external = market_root.get("detached_house_external_works_eur") or {}
    detached_house_total_build = market_root.get("detached_house_total_build_cost_eur") or {}
    detached_house_total_sale = market_root.get("detached_house_total_sale_eur") or {}
    detached_house_total_rent = market_root.get("detached_house_total_rent_eur_month") or {}
    ...
    def economics_for(
        ...
        specific_cost = construction_by_type.get(property_type) or {}
        specific_low = _usable_number(specific_cost.get("low"))
        specific_high = _usable_number(specific_cost.get("high"))
        if property_type == "house":
            local_cost_low = specific_low or cost_low * HOUSE_FALLBACK_COST_MULTIPLIER_LOW
            local_cost_high = specific_high or cost_high * HOUSE_FALLBACK_COST_MULTIPLIER_HIGH
            costed_area_factor = HOUSE_BUILT_AREA_FACTOR
        else:
            local_cost_low = specific_low or cost_low
            local_cost_high = specific_high or cost_high
            costed_area_factor = SCREENING_BUILT_AREA_FACTOR
        local_cost_low, local_cost_high = min(local_cost_low, local_cost_high), max(local_cost_low, local_cost_high)

        costed_area = gross * costed_area_factor
        external_low = 0.0
        external_high = 0.0
        if property_type == "house":
            external_low = _usable_number(detached_house_external.get("low")) or 0.0
            external_high = _usable_number(detached_house_external.get("high")) or external_low
            if count_h > 1.0:
                external_low *= 1.20
                external_high *= 1.35
        hard_low = costed_area * local_cost_low + external_low
        hard_high = costed_area * local_cost_high + external_high

        gdv_low, gdv_high = net_low * sale_low, net_high * sale_high
        sale_valuation_basis = "saleable_area_times_local_rate"
        build_valuation_basis = "constructed_area_times_tier_rate_plus_external_works"
        if property_type == "house" and count_h <= 1.0:
            direct_build_low = _usable_number(detached_house_total_build.get("low"))
            direct_build_high = _usable_number(detached_house_total_build.get("high"))
            if direct_build_low is not None and direct_build_high is not None:
                hard_low, hard_high = min(direct_build_low, direct_build_high), max(direct_build_low, direct_build_high)
                build_valuation_basis = "subject_site_whole_villa_total_build_cost_primary"
            direct_low = _usable_number(detached_house_total_sale.get("low"))
            direct_high = _usable_number(detached_house_total_sale.get("high"))
            if direct_low is not None and direct_high is not None:
                # Whole-property evidence is primary for a single luxury villa.
                # Do not let an ordinary house €/m² calculation dilute it.
                gdv_low, gdv_high = min(direct_low, direct_high), max(direct_low, direct_high)
                sale_valuation_basis = "subject_site_whole_villa_total_value_primary"

        non_land_before_sales_low = hard_low * (1 + SCREENING_SOFT_COST_PERCENT / 100.0)
        non_land_before_sales_high = hard_high * (1 + SCREENING_SOFT_COST_PERCENT / 100.0)
        non_land_low = non_land_before_sales_low + gdv_low * SCREENING_SALES_COST_PERCENT / 100.0
        non_land_high = non_land_before_sales_high + gdv_high * SCREENING_SALES_COST_PERCENT / 100.0
        ...
        rent_valuation_basis = "saleable_area_times_local_rent_rate"
        if property_type == "house" and count_h <= 1.0:
            direct_rent_low = _usable_number(detached_house_total_rent.get("low"))
            direct_rent_high = _usable_number(detached_house_total_rent.get("high"))
            if direct_rent_low is not None and direct_rent_high is not None:
                monthly_low, monthly_high = min(direct_rent_low, direct_rent_high), max(direct_rent_low, direct_rent_high)
                rent_valuation_basis = "subject_site_whole_villa_monthly_rent_primary"
        target_factor = 1.20
        residual_low = max(
            (gdv_low * (1.0 - SCREENING_SALES_COST_PERCENT / 100.0)) / target_factor
            - non_land_before_sales_high,
            0.0,
        )
        residual_high = max(
            (gdv_high * (1.0 - SCREENING_SALES_COST_PERCENT / 100.0)) / target_factor
            - non_land_before_sales_low,
            0.0,
        )
        return {
            "status": "calculated",
            ...
            "construction_cost_eur_m2_range": {"low": round(local_cost_low, 2), "high": round(local_cost_high, 2)},
            "costed_built_area_m2": round(costed_area, 1),
            "costed_built_area_factor": round(costed_area_factor, 3),
            "external_works_range_eur": {"low": round(external_low), "high": round(external_high)},
            "build_cost_range_eur": {"low": round(hard_low), "high": round(hard_high)},
            "build_valuation_basis": build_valuation_basis,
            "sale_valuation_basis": sale_valuation_basis,
            "rent_valuation_basis": rent_valuation_basis,
    ...
    residual_candidates = []
    for card in cards:
        residual = (card.get("economics") or {}).get("residual_land_value_at_20pct_target_poc_eur") or {}
        r_low, r_high = _num(residual.get("low")), _num(residual.get("high"))
        if r_low is None or r_high is None or max(r_low, r_high) <= 0:
            continue
    ...
    best_residual = max(
        residual_candidates,
        key=lambda item: (item["low"] + item["high"]) / 2.0,
        default=None,
    )

    expert_land_total = market_root.get("expert_land_total_eur") or {}
    expert_land_low = _num(expert_land_total.get("low"))
    expert_land_high = _num(expert_land_total.get("high"))
    if expert_land_low is not None and expert_land_high is not None:
        value_estimate = {
            "low_eur": round(min(expert_land_low, expert_land_high)),
            "high_eur": round(max(expert_land_low, expert_land_high)),
            "method": "expert_calibrated_exact_micro_market_total",
            "market_calibration_percent": 0.0,
            "land_evidence_status": land_analysis.get("evidence_status"),
            "land_confidence": land_analysis.get("confidence") or "high",
            "land_comparable_count": land_analysis.get("relevant_observation_count"),
            "best_use_residual_context": best_residual,
        }
    elif parcel_area and land_low_m2 is not None and land_high_m2 is not None:
        # Restore the older, better-performing land method: market land €/m²
        # multiplied by the actual parcel area. The only automatic adjustment is
        # a transparent 10% default calibration because the earlier version was
        # slightly high. Do not suppress prime plots with generic size discounts
        # or a development-residual cap.
        calibration_percent = min(max(LAND_MARKET_CALIBRATION_PERCENT, 0.0), 25.0)
        calibration_factor = 1.0 - calibration_percent / 100.0
        direct_low = parcel_area * min(land_low_m2, land_high_m2) * calibration_factor
        direct_high = parcel_area * max(land_low_m2, land_high_m2) * calibration_factor
        focused_land = str(land_analysis.get("evidence_status") or "") == "focused_exact_micro_market_land_research"
        value_estimate = {
            "low_eur": round(max(direct_low, 0.0)),
            "high_eur": round(max(direct_high, direct_low, 0.0)),
            "method": (
                "focused_micro_market_land_listings_calibrated"
                if focused_land
                else "nearby_land_listings_legacy_method_calibrated"
            ),
            "market_calibration_percent": round(calibration_percent, 1),
            "raw_land_asking_eur_m2": {
                "low": round(min(land_low_m2, land_high_m2)),
                "high": round(max(land_low_m2, land_high_m2)),
            },
            "land_evidence_status": land_analysis.get("evidence_status"),
            "land_confidence": land_analysis.get("confidence"),
            "land_comparable_count": land_analysis.get("relevant_observation_count"),
            "best_use_residual_context": best_residual,
        }
        ...
        value_estimate = {
            "low_eur": round(best_residual["low"] * 0.90),
            "high_eur": round(best_residual["high"]),
            "method": "best_use_development_residual_fallback",
            "best_use_residual_context": best_residual,
        }
    ...
    if value_estimate:
        ...
        if value_estimate.get("method") != "expert_calibrated_exact_micro_market_total":
            plot_low, plot_high = _screening_pair(
                plot_low,
                plot_high,
                max_half_width_percent=10.0,
            )
            value_estimate["range_method"] = "central_screening_band_v2"
        else:
            value_estimate["range_method"] = "expert_micro_market_range_preserved"
        value_estimate["low_eur"] = round(plot_low)
        value_estimate["high_eur"] = round(plot_high)
        for card in cards:
            ...
            non_land_low, non_land_high = _screening_pair(
                non_land_low,
                non_land_high,
                max_half_width_percent=7.0,
            )
            gdv_low, gdv_high = _screening_pair(
                gdv_low,
                gdv_high,
                max_half_width_percent=8.0,
            )
            ...
            total_low, total_high = _screening_pair(
                total_low,
                total_high,
                max_half_width_percent=7.0,
            )
            ...
            gdv_mid = (gdv_low + gdv_high) / 2.0
            expected_profit = gdv_mid - total_mid
            raw_profit_low = gdv_low - total_high
            raw_profit_high = gdv_high - total_low
            expected_roi = expected_profit / total_mid * 100.0 if total_mid > 0 else None
            raw_roi_low = raw_profit_low / total_high * 100.0 if total_high > 0 else None
            raw_roi_high = raw_profit_high / total_low * 100.0 if total_low > 0 else None
            downside_loss_risk = raw_profit_low < 0

            economics["plot_price_range_eur"] = {"low": round(plot_low), "high": round(plot_high)}
            economics["non_land_development_cost_range_eur"] = {"low": round(non_land_low), "high": round(non_land_high)}
            economics["gdv_range_eur"] = {"low": round(gdv_low), "high": round(gdv_high)}
            economics["total_investment_range_eur"] = {"low": round(total_low), "high": round(total_high)}
            economics["expected_total_investment_eur"] = round(total_mid)
            economics["expected_gdv_eur"] = round(gdv_mid)
            economics["expected_profit_eur"] = round(expected_profit)
            economics["expected_roi_percent"] = round(expected_roi, 1) if expected_roi is not None else None
            economics["downside_loss_risk"] = downside_loss_risk
            economics["financial_range_method"] = "central_correlated_screening_v2"

            if expected_profit <= 0 or expected_roi is None or expected_roi <= 0:
                economics["estimated_profit_range_eur"] = None
                economics["roi_range_percent"] = None
                economics["feasibility_status"] = "not_viable_at_current_assumptions"
            else:
                profit_half_cap = max(expected_profit * 0.22, 25_000.0)
                profit_low = max(0.0, raw_profit_low, expected_profit - profit_half_cap)
                profit_high = max(profit_low, min(raw_profit_high, expected_profit + profit_half_cap))
                roi_low = max(0.0, raw_roi_low or 0.0, expected_roi - 5.0)
                roi_high = max(roi_low, min(raw_roi_high or expected_roi, expected_roi + 5.0))
                economics["estimated_profit_range_eur"] = {"low": round(profit_low), "high": round(profit_high)}
                economics["roi_range_percent"] = {"low": round(roi_low, 1), "high": round(roi_high, 1)}
                economics["feasibility_status"] = "positive_expected_return_with_downside_risk" if downside_loss_risk else "positive_expected_return"
            ...
            if annual_low is not None and annual_high is not None:
                annual_low, annual_high = _screening_pair(
                    annual_low,
                    annual_high,
                    max_half_width_percent=8.0,
                )
                economics["whole_scheme_annual_rent_range_eur"] = {"low": round(annual_low), "high": round(annual_high)}
                economics["whole_scheme_monthly_rent_range_eur"] = {"low": round(annual_low / 12.0), "high": round(annual_high / 12.0)}
    ...
    viable_modelled = [
        card for card in financially_modelled
        if _num((card.get("economics") or {}).get("expected_profit_eur")) is not None
        and (_num((card.get("economics") or {}).get("expected_profit_eur")) or 0) > 0
    ]
    best_profit = max(viable_modelled, key=lambda card: _num((card.get("economics") or {}).get("expected_profit_eur")) or -math.inf, default=None)
    ...
    return {
        "engine_version": "plana-investment-v1-prime-villa-2026-07-18",
...
HOUSE_BUILT_AREA_FACTOR = float(os.getenv("PLANA_HOUSE_BUILT_AREA_FACTOR", "1.20"))
HOUSE_FALLBACK_COST_MULTIPLIER_LOW = float(os.getenv("PLANA_HOUSE_COST_MULTIPLIER_LOW", "1.20"))
HOUSE_FALLBACK_COST_MULTIPLIER_HIGH = float(os.getenv("PLANA_HOUSE_COST_MULTIPLIER_HIGH", "1.45"))
LAND_ASKING_DISCOUNT_PERCENT = float(os.getenv("PLANA_LAND_ASKING_DISCOUNT_PERCENT", "10"))
LAND_RESIDUAL_CAP_MULTIPLIER = float(os.getenv("PLANA_LAND_RESIDUAL_CAP_MULTIPLIER", "1.10"))
LAND_MARKET_CALIBRATION_PERCENT = float(os.getenv("PLANA_LAND_MARKET_CALIBRATION_PERCENT", "10"))
...
""".strip()
    ...
    cached = PARCEL_AI_RESPONSE_CACHE.get(cache_key)
    if cached and time.time() - cached[0] < AI_RESPONSE_CACHE_TTL_SECONDS:
        return {**cached[1], "cached": True}
    ...
    try:
        ...
        PARCEL_AI_RESPONSE_CACHE[cache_key] = (time.time(), result)
        return result
    except Exception as exc:
...
async def site_extra(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # Official DLS is the primary source. Supabase is only a resilience fallback
    # for individual DLS layers that fail or time out.
    if LIVE_DLS_FIRST:
        live_attempted = True
        ...
        for (layer_id, layer_name), result in zip(layer_items, layer_results):
            ...
            spatial_checks[str(layer_id)] = {
                "layer_name": layer_name,
                **result,
                "source": "official_dls_live",
            }
        live_complete = all(bool(spatial_checks.get(str(layer_id), {}).get("ok")) for layer_id, _ in layer_items)

    # Read the stored completed DLS cycle only when live DLS was disabled or when
    # one or more live layers failed. Successful live results are never replaced.
    if not live_complete:
        ...
        if supabase:
            try:
                response = await asyncio.to_thread(
                    lambda: supabase.rpc(
                        "get_plana_dls_support_for_geometry",
                        {"p_geometry": geometry, "p_layer_ids": list(SPECIAL_LAYERS)},
                    ).execute()
                )
                stored = response.data or {}
                if isinstance(stored, dict) and stored.get("complete"):
                    for layer_id, layer_name in layer_items:
                        existing = spatial_checks.get(str(layer_id)) or {}
                        if existing.get("ok"):
                            continue
                        ...
                        features = [
                            ...
                            for item in layer.get("features") or []
                        ]
                        spatial_checks[str(layer_id)] = {
                            "layer_name": layer_name,
                            "ok": True,
                            "features": features,
                            "source": "plana_dls_completed_cycle_fallback",
                            "completed_cycle_key": layer.get("completed_cycle_key"),
                        }
                        stored_used = True
            except Exception as exc:
                print(f"PLANA stored DLS support fallback: {exc!r}", file=sys.stderr)

    # When live-first is disabled, or the database did not cover a missing layer,
    # make a final official-DLS attempt for only the unresolved layers.
    unresolved = [
        (layer_id, layer_name)
        for layer_id, layer_name in layer_items
        if not (spatial_checks.get(str(layer_id)) or {}).get("ok")
    ]
    if unresolved:
        live_attempted = True
        layer_results = await asyncio.gather(
            *(query_layer_intersections(layer_id, parcel_feature) for layer_id, _ in unresolved),
            return_exceptions=True,
        )
        for (layer_id, layer_name), result in zip(unresolved, layer_results):
            if isinstance(result, Exception):
                result = {"ok": False, "error": str(result)}
            spatial_checks[str(layer_id)] = {
                "layer_name": layer_name,
                **result,
                "source": "official_dls_live" if result.get("ok") else "official_dls_live_failed",
            }
...
<div class="app">
  ...
  <aside class="panel">
    <div id="empty" class="empty">
      <div><div class="empty-icon">⌂</div><h1>Choose a plot</h1><p>Search a location or click the exact cadastral parcel.</p></div>
    </div>
...
function roiDisplay(e){if(e?.feasibility_status==="not_viable_at_current_assumptions")return "Not viable";const value=rangePct(e?.roi_range_percent);return e?.downside_loss_risk&&value!=="—"?`${value} · loss risk`:value}
function profitDisplay(e){return e?.feasibility_status==="not_viable_at_current_assumptions"?"Loss at current assumptions":rangeMoney(e?.estimated_profit_range_eur)}
...
let parcelLayer=null,amenityLayer=L.layerGroup().addTo(map),current=null,planning=null,opportunity=null,proposals=null,locationInfo=null,siteExtra=null,selectionId=0,searchTimer=null,searchResults=[],searchSession=crypto.randomUUID?crypto.randomUUID():String(Date.now()),selectionBusy=false,planningLoading=false,planningRequested=false;
...
function allowedUseChips(){const uses=proposals?.allowed_uses||[];if(!uses.length)return `<div class="empty-result">Planning uses are being resolved from the live DLS zone and verified zone registry.</div>`;return `<div class="chips">${uses.map(use=>`<span class="chip ${use.status==='allowed'?'':'maybe'}">${esc(use.label)} · ${use.status==='allowed'?'Allowed':'Potentially allowed'}</span>`).join("")}</div>`}
...
function optionCard(option){const e=option.economics||{},badges=badgesFor(option),topMetrics=(option.metrics||[]).slice(0,3);return `<article class="option-card ${badges.length?'best':''}" data-option="${esc(option.id)}"><div class="option-top"><div class="option-title"><h3>${esc(option.label)}</h3><div class="badges">${badges.map(b=>`<span class="badge">${esc(b)}</span>`).join("")}</div></div><div class="option-headline">${esc(option.headline)}</div><div class="option-metrics">${topMetrics.map(item=>`<div class="option-mini"><span>${esc(item.label)}</span><b>${esc(item.value??"—")} ${esc(item.unit||"")}</b></div>`).join("")}</div></div><div class="option-details">${e.status==='calculated'?`<div class="finance-grid"><div class="finance-item"><span>Build cost</span><b>${rangeMoney(e.build_cost_range_eur)}</b></div><div class="finance-item"><span>Total investment</span><b>${rangeMoney(e.total_investment_range_eur)}</b></div><div class="finance-item"><span>Completed sale value</span><b>${rangeMoney(e.gdv_range_eur)}</b></div><div class="finance-item"><span>Profit</span><b>${profitDisplay(e)}</b></div><div class="finance-item"><span>Sale ROI</span><b>${roiDisplay(e)}</b></div><div class="finance-item"><span>Rental yield</span><b>${rangePct(e.gross_rental_yield_range_percent)}</b></div><div class="finance-item"><span>Monthly rent</span><b>${rangeMoney(e.whole_scheme_monthly_rent_range_eur)}</b></div><div class="finance-item"><span>Sale per unit</span><b>${rangeMoney(e.sale_per_unit_range_eur)}</b></div></div>`:`<div class="empty-result">Market data for this use is not available yet.</div>`}</div></article>`}
function renderOptions(){const list=proposals?.development_options||[];$("options").innerHTML=list.length?`<div class="card"><h2>Development options</h2><div class="empty-result" style="padding-top:0">Select an option to see its investment figures.</div></div>${list.map(optionCard).join("")}`:`<div class="card"><h2>Development options</h2><div class="empty-result">Options are being recalculated from the selected plot.</div></div>`;document.querySelectorAll(".option-card").forEach(card=>card.querySelector(".option-top").onclick=()=>card.classList.toggle("open"))}
...
<div class="card"><h2>Sale and rental comparison</h2>${rows.length?rows.map(option=>{const e=option.economics;return `<div class="wide-metric"><span>${esc(option.label)}</span><b>${rangeMoney(e.gdv_range_eur)}<br><small>Build ${rangeMoney(e.build_cost_range_eur)} · ${roiDisplay(e)} ROI</small></b></div>`}).join(""):`<div class="empty-result">Investment figures will appear when market evidence is available.</div>`}</div>
...
function renderNearby(){const amenities=locationInfo?.amenities||[],radius=locationInfo?.radius_cap_m||1500,market=opportunity?.market_analysis||{},property=market.property_analyses||{};const marketCards=[["Apartments",property.apartment],["Houses",property.house],["Offices",property.office],["Shops",property.shop],["Land",property.land]].filter(([,value])=>value);$("nearby").innerHTML=`
<div class="card"><h2>Nearest amenities</h2><div class="empty-result" style="padding-top:0">Only the nearest places within ${fmt(radius/1000,1)} km are shown.</div>${amenities.length?`<div class="amenity-grid">${amenities.map(amenityButton).join("")}</div>`:`<div class="empty-result">Nearby places are being refreshed.</div>`}</div>
<div class="card"><h2>Average market estimates</h2>${marketCards.map(([label,value])=>`<div class="wide-metric"><span>${label}</span><b>${value.sale_price_per_m2?`${money(value.sale_price_per_m2.low)}–${money(value.sale_price_per_m2.high)} / m²`:"Estimated in investment options"}${value.rent_per_m2_month?`<br><small>€${fmt(value.rent_per_m2_month.low,1)}–€${fmt(value.rent_per_m2_month.high,1)} rent / m²</small>`:""}</b></div>`).join("")}</div>`;bindDynamic()}
...
function activateTab(name){document.querySelectorAll(".tab").forEach(tab=>tab.classList.toggle("active",tab.dataset.tab===name));document.querySelectorAll(".tab-page").forEach(page=>page.classList.toggle("active",page.id===name));if(name==="build"||name==="options"){planningRequested=true;if(!selectionBusy)ensurePlanningAnalysis()}}
document.querySelectorAll(".tab").forEach(tab=>tab.onclick=()=>activateTab(tab.dataset.tab));

function showAmenity(category){amenityLayer.clearLayers();const group=(locationInfo?.amenities||[]).find(item=>item.category===category);if(!group)return;(group.places||[]).forEach(place=>L.marker([place.lat,place.lon]).bindPopup(`<b>${esc(place.name)}</b><br>${fmt(place.distance_m,0)} m`).addTo(amenityLayer));if(group.places?.length){const bounds=L.latLngBounds(group.places.map(place=>[place.lat,place.lon]));if(bounds.isValid())map.fitBounds(bounds.pad(.18),{maxZoom:16})}}

function mergeProposalPayload(previous,refined){if(!refined)return previous;const oldList=previous?.development_options||[],newList=refined?.development_options||[];if(oldList.length&&!newList.length)return {...previous,...refined,development_options:oldList,investment_summary:previous.investment_summary||refined.investment_summary};return refined}
async function ensurePlanningAnalysis(){if(!current||planning||planningLoading)return;if(selectionBusy){planningRequested=true;return}const id=selectionId;planningLoading=true;try{const planningData=await post("/api/parcel-planning-analysis",{parcel_id:current.parcel.parcel_id},90000);if(id!==selectionId)return;planning=planningData;const refined=await post("/api/parcel-proposals",{parcel_id:current.parcel.parcel_id,planning_analysis:planning,opportunity_analysis:opportunity},35000);if(id!==selectionId)return;proposals=mergeProposalPayload(proposals,refined);renderAll()}catch(error){console.warn(error)}finally{if(id===selectionId)planningLoading=false}}

async function selectParcel(lat,lon){const id=++selectionId;selectionBusy=true;planningRequested=false;planningLoading=false;amenityLayer.clearLayers();setLoading(true,"Opening plot…");try{const basic=await get(`/api/parcel-basic?lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}`,30000);if(id!==selectionId)return;if(parcelLayer)map.removeLayer(parcelLayer);parcelLayer=L.geoJSON(basic.parcel_feature,{style:{color:"#173f2b",weight:3,fillColor:"#6fa482",fillOpacity:.22}}).addTo(map);try{map.fitBounds(parcelLayer.getBounds().pad(.22),{maxZoom:19})}catch{}const details=await get(`/api/parcel-details?parcel_id=${basic.parcel_id}`,45000);if(id!==selectionId)return;current={...details,parcel_feature:basic.parcel_feature,geometry_metrics:basic.geometry_metrics||details.geometry_metrics||{}};planning=null;opportunity={market_analysis:details.initial_market_analysis||{}};proposals=details.initial_proposals||null;locationInfo=null;siteExtra=null;$("empty").classList.add("hidden");$("content").classList.remove("hidden");$("parcelTitle").textContent=`Plot ${current.parcel?.parcel_number||current.parcel?.parcel_id||"—"}`;$("parcelPlace").textContent=locationLabel(current.parcel||{});renderAll();setLoading(true,"Calculating investment options…");const centroidLat=num(current.geometry_metrics?.centroid_lat)??lat,centroidLon=num(current.geometry_metrics?.centroid_lon)??lon;const oppPromise=post("/api/parcel-opportunity-analysis",{parcel_id:current.parcel.parcel_id,assumptions:{},centroid_lat:centroidLat,centroid_lon:centroidLon},120000).then(data=>{if(id!==selectionId)return;opportunity=data;proposals=mergeProposalPayload(proposals,data.proposals);renderAll()}).catch(error=>console.warn(error));const locationPromise=get(`/api/location-intelligence?lat=${centroidLat}&lon=${centroidLon}`,25000).then(data=>{if(id!==selectionId)return;locationInfo=data;renderAll()}).catch(error=>console.warn(error));const extraPromise=post("/api/site-extra",{parcel_feature:basic.parcel_feature},18000).then(data=>{if(id!==selectionId)return;siteExtra=data}).catch(error=>console.warn(error));await Promise.allSettled([oppPromise,locationPromise,extraPromise]);if(id!==selectionId)return;selectionBusy=false;setLoading(false);if(planningRequested)ensurePlanningAnalysis()}catch(error){if(id!==selectionId)return;selectionBusy=false;setLoading(false);alert(error.message||String(error))}}
...
$("searchInput").addEventListener("input",()=>{clearTimeout(searchTimer);searchTimer=setTimeout(loadSuggestions,260)});$("searchInput").addEventListener("keydown",async event=>{if(event.key==="Enter"){event.preventDefault();if(searchResults.length)await chooseSuggestion(searchResults[0]);else{await loadSuggestions();if(searchResults.length)await chooseSuggestion(searchResults[0])}}if(event.key==="Escape")$("suggestions").classList.add("hidden")});document.addEventListener("click",event=>{if(!event.target.closest(".search-wrap"))$("suggestions").classList.add("hidden")});
