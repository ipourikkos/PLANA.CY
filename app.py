...
def normalize_parcel_details(
    ...
    try:
        ...
        if zones:
            if len(zones) == 1:
                zone = zones[0]
                if zone.get("density_percent") is not None:
                    max_floor_area = round(area * float(zone["density_percent"]) / 100, 2)
                if zone.get("coverage_percent") is not None:
                    max_ground_coverage = round(area * float(zone["coverage_percent"]) / 100, 2)
            else:
                floor_total = 0.0
                coverage_total = 0.0
                floor_ok = coverage_ok = False
                for zone in zones:
                    overlap = zone.get("overlap_percent")
                    if overlap is None:
                        continue
                    affected_area = area * float(overlap) / 100
                    if zone.get("density_percent") is not None:
                        floor_total += affected_area * float(zone["density_percent"]) / 100
                        floor_ok = True
                    if zone.get("coverage_percent") is not None:
                        coverage_total += affected_area * float(zone["coverage_percent"]) / 100
                        coverage_ok = True
                if floor_ok:
                    max_floor_area = round(floor_total, 2)
                if coverage_ok:
                    max_ground_coverage = round(coverage_total, 2)
    except Exception:
        pass
...
async function selectSite(lat,lon){
 ...
 try{
   ...
   $("potential").innerHTML=`<div class="grid">
     ${card("Maximum floor area",currentSite.development_potential?.theoretical_max_floor_area_m2,true," m²")}
     ${card("Maximum footprint",currentSite.development_potential?.theoretical_max_ground_coverage_m2,true," m²")}
   </div>
   <p class="muted">Calculated from parcel area and planning coefficients. These are theoretical indicators, not guaranteed development rights.</p>`;
