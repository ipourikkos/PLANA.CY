...
def build_viable_development_options(
    ...
    front, side, rear = _num(pinputs.get("front_setback_m")), _num(pinputs.get("side_setback_m")), _num(pinputs.get("rear_setback_m"))
    working_setback = False
    if side is None or rear is None:
        side, rear, working_setback = side if side is not None else 3.0, rear if rear is not None else 3.0, True
    if front is None:
        front, working_setback = 3.0, True

    longest, shortest = _num(geometry.get("longest_edge_m")), _num(geometry.get("shortest_edge_m"))
    ...
    footprint_values = [x for x in (geometric_envelope, coverage_cap) if x is not None]
    ...
    allowed = [x for x in (planning_analysis.get("allowed_development_options") or _zone_option_fallback(parcel_details)) if x.get("status") != "not_supported"]
    ...
    def economics_for(kind: str, count_low: float | None, count_high: float | None) -> dict[str, Any]:
        property_type = "apartment" if kind == "apartments" else ("house" if kind == "house" else None)
        analysis = property_analyses.get(property_type) if property_type else None
        if not analysis or not gross:
            return {"status": "withheld_no_use_specific_market_evidence" if kind == "mixed_use" else "insufficient_market_data", "property_type": property_type}
        ...
        if kind == "apartments" and balanced:
            net_low = _num(balanced.get("allocated_unit_area_low_m2"))
            net_high = _num(balanced.get("allocated_unit_area_high_m2"))
        else:
            net_low, net_high = gross * efficiency_low, gross * efficiency_high
        if sale_low is None or sale_high is None or net_low is None or net_high is None:
            return {"status": "insufficient_use_specific_sale_evidence", "property_type": property_type, "market_confidence": analysis.get("confidence"), "observation_count": analysis.get("relevant_observation_count")}
        costed_area = gross * SCREENING_BUILT_AREA_FACTOR
        hard_low, hard_high = costed_area * SCREENING_COST_LOW_EUR_M2, costed_area * SCREENING_COST_HIGH_EUR_M2
        ...
        poc_low = (gdv_low - non_land_high) / non_land_high * 100.0 if non_land_high > 0 else None
        poc_high = (gdv_high - non_land_low) / non_land_low * 100.0 if non_land_low > 0 else None
        return {
            "status": "screening_estimate_from_use_specific_asking_market",
            "property_type": property_type,
            "market_confidence": analysis.get("confidence"),
            "observation_count": analysis.get("relevant_observation_count"),
            "primary_source_count": analysis.get("primary_source_count"),
            ...
            "pre_land_profit_on_cost_range_percent": ({"low": round(min(poc_low, poc_high), 1), "high": round(max(poc_low, poc_high), 1)} if poc_low is not None and poc_high is not None else None),
            "basis": "Use-specific asking-price/rent comparables plus configurable PLANA screening cost assumptions. Pre-land POC excludes acquisition cost and is not an investment return forecast.",
        }

    option_cards: list[dict[str, Any]] = []
    for item in allowed:
        kind, status = item.get("type"), item.get("status") or "conditional"
        if kind == "house":
            density_count = max(1, int((gross or 220.0) // 220.0)) if gross else 1
            land_count = max(1, int(parcel_area // 350.0)) if parcel_area else density_count
            count = min(density_count, land_count, 4)
            card = {"type": "house", "label": "Houses", "status": status, "headline": f"About {count} house{'s' if count != 1 else ''}", "metrics": [{"label": "Density-counted area", "value": round(gross, 1) if gross else None, "unit": "m²"}, {"label": "Working footprint cap", "value": round(footprint_cap, 1) if footprint_cap else None, "unit": "m²"}, {"label": "Parking signal", "value": count * 2, "unit": "spaces"}], "maximisation": ["Use qualifying basement support/parking areas outside density where Order 4/2026 conditions are met.", "Use qualifying balconies and covered verandas within the 25% per-floor exclusion framework.", "Keep uncovered garden/pool space within the remaining open ground area."], "reason": item.get("reason")}
            card["economics"] = economics_for(kind, count, count)
        elif kind == "apartments":
            low, high = (balanced or {}).get("unit_count_low"), (balanced or {}).get("unit_count_high")
            parking_low, parking_high = (balanced or {}).get("parking_spaces_low"), (balanced or {}).get("parking_spaces_high")
            card = {"type": "apartments", "label": "Apartments", "status": status, "headline": f"{low}–{high} units" if low is not None and high is not None else "Apartment capacity scenario", "metrics": [{"label": "Net saleable", "value": f"{balanced.get('net_saleable_area_low_m2')}–{balanced.get('net_saleable_area_high_m2')}" if balanced else None, "unit": "m²"}, {"label": "Parking signal", "value": f"{parking_low}–{parking_high}" if parking_low is not None else None, "unit": "spaces"}, {"label": "Covered balcony capacity", "value": round(covered_balcony_allowance, 1) if covered_balcony_allowance else None, "unit": "m²"}], "maximisation": ["Test parking in basement/pilotis because qualifying parking can be excluded from density under Order 4/2026 conditions.", "Allocate qualifying balconies/covered verandas per floor before treating extra covered external area as density-counted.", "Use entrance, circulation and support-space exclusions only where the specific Order 4/2026 conditions are satisfied."], "reason": item.get("reason"), "unit_mix_low": (balanced or {}).get("unit_counts_low"), "unit_mix_high": (balanced or {}).get("unit_counts_high")}
            card["economics"] = economics_for(kind, _num(low), _num(high))
        elif kind == "mixed_use":
            card = {"type": "mixed_use", "label": "Mixed use", "status": status, "headline": "20% commercial / 80% residential test", "metrics": [{"label": "Commercial test area", "value": round((gross or 0) * .20, 1) if gross else None, "unit": "m²"}, {"label": "Residential test area", "value": round((gross or 0) * .80, 1) if gross else None, "unit": "m²"}, {"label": "Working footprint cap", "value": round(footprint_cap, 1) if footprint_cap else None, "unit": "m²"}], "maximisation": ["Keep the ground-floor commercial share explicit and re-test parking by use.", "Do not apply apartment asking-price evidence to the commercial share.", "Do not assume the 20/80 split is an entitlement; it is a feasibility test within the supported use category."], "reason": item.get("reason"), "economics": {"status": "withheld_no_commercial_market_evidence", "basis": "Mixed-use GDV is withheld until commercial sale/rent evidence is normalized."}}
        else:
            card = {"type": kind, "label": str(kind).replace("_", " ").title(), "status": status, "headline": "Use-specific feasibility test", "metrics": [{"label": "Density-counted area", "value": round(gross, 1) if gross else None, "unit": "m²"}, {"label": "Working footprint cap", "value": round(footprint_cap, 1) if footprint_cap else None, "unit": "m²"}], "maximisation": ["Apply the use-specific parking and support-space rules before converting capacity into a programme."], "reason": item.get("reason"), "economics": {"status": "withheld_no_use_specific_market_model"}}
        option_cards.append(card)

    residuals = [card.get("economics", {}).get("residual_land_value_at_20pct_target_poc_eur") for card in option_cards]
    residuals = [item for item in residuals if isinstance(item, dict) and item.get("low") is not None and item.get("high") is not None]
    value_estimate = {"low_eur": min(int(x["low"]) for x in residuals), "high_eur": max(int(x["high"]) for x in residuals), "basis": "Residential residual land screening at 20% target profit-on-cost; use-specific asking-market evidence"} if residuals else None
    dls_value = _num(parcel.get("price_2021"))
    return {
        "engine_version": "plana-proposals-v2",
        "setbacks": {"front_m": front, "side_m": side, "rear_m": rear, "status": "working_assumption" if working_setback else setback_status, "note": "3.0 m is used only as a preliminary working envelope where the applicable Development Plan baseline distance has not yet been confirmed." if working_setback else pinputs.get("setback_note"), "envelope_width_m": round(envelope_w, 2) if envelope_w is not None else None, "envelope_length_m": round(envelope_l, 2) if envelope_l is not None else None, "working_footprint_cap_m2": round(footprint_cap, 2) if footprint_cap is not None else None},
        "space_strategy": {"density_counted_floor_area_m2": round(gross, 2) if gross else None, "planning_max_floors": floors, "indicative_density_area_per_floor_m2": round(per_floor_density, 2) if per_floor_density else None, "indicative_covered_balcony_veranda_allowance_m2": covered_balcony_allowance, "remaining_open_ground_m2": round(open_ground, 2) if open_ground is not None else None},
        "development_options": option_cards,
        "value_estimate": value_estimate,
        "dls_2021_value_eur": dls_value,
        "value_status": "screening_estimate" if value_estimate else ("official_context_only" if dls_value else "insufficient_use_specific_market_data"),
