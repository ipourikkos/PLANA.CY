...
APP_BUILD = "2026-07-16-range-nearby-cost-fix-v4"
...
MARKET_RESEARCH_MODEL = os.getenv("OPENAI_MARKET_RESEARCH_MODEL", "gpt-5.6-terra")
MARKET_STRUCTURING_MODEL = os.getenv("OPENAI_MARKET_STRUCTURING_MODEL", "gpt-5.6-luna")
...
MARKET_ANALYSIS_CACHE_TTL_SECONDS = max(3600, int(os.getenv("PLANA_MARKET_RESEARCH_CACHE_TTL_SECONDS", "86400")))
...
AI_RESPONSE_CACHE_TTL_SECONDS = max(900, int(os.getenv("PLANA_AI_RESPONSE_CACHE_TTL_SECONDS", "21600")))
...
async def parcel_details(parcel_id: int = Query(gt=0)) -> dict[str, Any]:
    ...
    fallback_market = _calibrate_market_analysis_ranges(_fallback_market_analysis(details))
...
def _fallback_market_analysis(parcel_details: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        **properties["apartment"],
        ...
        "construction_cost_eur_m2": {"low": defaults["construction"][0], "high": defaults["construction"][1]},
...
MARKET_RANGE_CALIBRATION_VERSION = "central-band-v2"
...
def _apply_live_research(base: dict[str, Any], live: LiveMarketResearchResult | None) -> dict[str, Any]:
    ...
    for kind, band in mapping.items():
        ...
        existing["sale_price_per_m2"] = _normalise_live_range(band.sale_low_eur_m2, band.sale_high_eur_m2, sale_fallback["low"], sale_fallback["high"])
        existing["rent_per_m2_month"] = _normalise_live_range(band.rent_low_eur_m2_month, band.rent_high_eur_m2_month, rent_fallback["low"], rent_fallback["high"])
    ...
    land["sale_price_per_m2"] = _normalise_live_range(live.land_sale_low_eur_m2, live.land_sale_high_eur_m2, land_fallback["low"], land_fallback["high"])
    land["evidence_status"] = "targeted_live_web_research"
    properties["land"] = land
    result["property_analyses"] = properties
    result.update(properties["apartment"])
    construction = result.get("construction_cost_eur_m2") or {"low": SCREENING_COST_LOW_EUR_M2, "high": SCREENING_COST_HIGH_EUR_M2}
    result["construction_cost_eur_m2"] = _normalise_live_range(live.construction_cost_low_eur_m2, live.construction_cost_high_eur_m2, construction["low"], construction["high"])
...
def _run_live_market_research(parcel_details: dict[str, Any]) -> LiveMarketResearchResult | None:
    ...
    prompt = f"""
Research the current property market around this exact Cyprus parcel location.
Location: quarter/locality={parcel.get('quarter')}; municipality={parcel.get('municipality')}; district={parcel.get('district')}.
Planning zone: {zones}. Parcel area: {parcel.get('parcel_extent_m2')} m².
Find current public asking-price evidence for residential land, apartments, houses, offices and shops, plus current Cyprus construction-cost evidence.
This live online research is the primary valuation evidence for PLANA; do not defer to stored database estimates.
Prioritise the exact locality, then municipality, then district. Use current Cyprus property portals, estate agents, developers, cost reports and official statistical context.
Return useful market ranges even when the exact locality is thin by broadening geographically. Do not use one outlier as the range. Distinguish asking prices from completed sales internally.
Your final response should be a concise research summary containing numeric low/high EUR per m² sale ranges and EUR per m² per month rent ranges for every property type, land EUR/m², and construction EUR/m².
"""
    ...
    structured = client.responses.parse(
        model=MARKET_STRUCTURING_MODEL,
        input=[
            {"role": "system", "content": "Extract realistic numeric Cyprus market ranges from the research. All fields are required. Order every low/high pair. Do not return zeros."},
            {"role": "user", "content": research.output_text},
        ],
        text_format=LiveMarketResearchResult,
    )
    return structured.output_parsed
...
async def build_hybrid_market_analysis(
    ...
    has_named_locality = bool(parcel.get("quarter") or parcel.get("municipality"))
    cache_key = json.dumps({
        "district": parcel.get("district"),
        "municipality": parcel.get("municipality"),
        "quarter": parcel.get("quarter"),
        "zone_codes": zone_codes,
        "lat_cell": round(centroid_lat, 2) if centroid_lat is not None and not has_named_locality else None,
        "lon_cell": round(centroid_lon, 2) if centroid_lon is not None and not has_named_locality else None,
        "hybrid": 2,
    ...
    supabase_result = await asyncio.to_thread(build_parcel_market_analysis, parcel_details, centroid_lat=centroid_lat, centroid_lon=centroid_lon)
    ...
    # Online-first market policy: fresh web research supplies the primary pricing
    # and construction ranges. Stored observations remain attached as supporting
    # comparables and become the fallback only if online research is unavailable.
    if LIVE_MARKET_ENABLED:
        try:
            live = await asyncio.wait_for(
                asyncio.to_thread(_run_live_market_research, parcel_details),
...
def build_viable_development_options(
    ...
    def economics_for(
        ...
        hard_low = costed_area * cost_low
        hard_high = costed_area * cost_high
        ...
        residual_low = _screening_residual(net_low, gross, sale_low, cost_high, 20.0)
        residual_high = _screening_residual(net_high, gross, sale_high, cost_low, 20.0)
        return {
            "status": "calculated",
            "property_type": property_type,
            ...
            "construction_cost_eur_m2_range": {"low": cost_low, "high": cost_high},
