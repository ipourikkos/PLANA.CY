...
def canonical_details_from_index(row: dict[str, Any]) -> dict[str, Any]:
    ...
    zones = [zone] if zone.get("zone") else []
    warnings = []
    ...
    result = {
        "parcel": {
            ...
            "house_no": None,
            "parcel_extent_m2": row.get("parcel_area_m2"),
            "price_2021": None,
            "price_2018": None,
            "price_1980": None,
            "valuation_change_percent": None,
            "is_preserved": bool(raw.get("is_preserved", False)),
            "is_ancient": bool(raw.get("is_ancient", False)),
        ...
        "development_potential": {
            ...
            "effective_density_percent": row.get("density_percent"),
            "effective_coverage_percent": row.get("coverage_percent"),
            "calculation_method": "PLANA precomputed parcel index",
            "area_basis_status": "indexed_official_geometry",
            "calculation_authority_status": "preliminary_screening",
            "multi_zone_policy_status": "primary_indexed_zone",
            "zone_overlap_total_percent": raw.get("primary_zone_overlap_percent"),
            "zone_overlap_complete": raw.get("primary_zone_overlap_percent") is not None,
            "calculation_warnings": [],
        },
        "registration_summary": {
            "total_related_records": 0,
            "by_type": {},
            "total_enclosed_extent_m2": row.get("existing_enclosed_m2"),
        ...
        "warnings": warnings,
        "building_summary": {
            "count": row.get("building_count") or 0,
            "features": [],
            "footprint_m2": row.get("building_footprint_m2"),
        },
        "contour_summary": {"terrain_range_m": row.get("terrain_range_m")},
        "spatial_checks": {
            "coast_protection": bool(raw.get("coast_protection_overlap", False)),
            "state_land": bool(raw.get("state_land_overlap", False)),
            "white_zone": bool(raw.get("white_zone_overlap", False)),
        },
        "index_metadata": {
            "source": "plana_parcels",
            "score_version": row.get("score_version"),
            "data_confidence": row.get("data_confidence"),
            "indexed_at": row.get("indexed_at"),
            "dls_refreshed_at": row.get("dls_refreshed_at"),
        },
    }
...
async def parcel_basic(
    ...
    # True fast path: never let a slow Supabase/PostgREST RPC hold the click.
    indexed = None
    try:
        indexed = await asyncio.wait_for(
            get_indexed_parcel_at_point(lat, lon),
            timeout=1.5,
        )
    except asyncio.TimeoutError:
        print(
            f"PLANA parcel-basic index timeout lat={lat} lon={lon}",
            file=sys.stderr,
        )
    except Exception as exc:
        print(
            f"PLANA parcel-basic index error lat={lat} lon={lon}: {exc!r}",
            file=sys.stderr,
        )
    ...
    if indexed:
        ...
        if parcel_feature:
            ...
            print(
                f"PLANA parcel-basic source=plana_index elapsed={elapsed:.2f}s "+
                f"parcel_id={indexed.get('parcel_id')}",
                file=sys.stderr,
            )
            return {
    ...
    # Direct official DLS Parcels layer fallback. Keep this under the frontend's
    # 14-second timeout so the UI never waits indefinitely.
    try:
        parcel_feature = await asyncio.wait_for(
            get_parcel_at_point(lat, lon),
            timeout=6.0,
        )
    except asyncio.TimeoutError:
        elapsed = time.perf_counter() - started
        print(
            f"PLANA parcel-basic DLS timeout elapsed={elapsed:.2f}s lat={lat} lon={lon}",
            file=sys.stderr,
        )
        raise HTTPException(
            status_code=504,
            detail="Official DLS parcel layer did not respond within the fast parcel timeout.",
        )
    ...
    elapsed = time.perf_counter() - started
    print(
        f"PLANA parcel-basic source=live_dls elapsed={elapsed:.2f}s parcel_id={sbpi}",
        file=sys.stderr,
    )
    return {
        "parcel_feature": parcel_feature,
...
# PLANA model fallbacks are used only when local scraped evidence is thin.
# They are screening assumptions, not formal valuations or observed transactions.
PLANA_DISTRICT_MODEL = {
    "limassol": {"sale": 3900.0, "rent": 18.0},
    "nicosia": {"sale": 2600.0, "rent": 12.5},
    "larnaca": {"sale": 2900.0, "rent": 14.0},
    "paphos": {"sale": 3200.0, "rent": 15.0},
    "famagusta": {"sale": 2800.0, "rent": 13.5},
    "ammochostos": {"sale": 2800.0, "rent": 13.5},
}

def _model_market_range(parcel_details: dict[str, Any]) -> dict[str, Any]:
    parcel = parcel_details.get("parcel") or {}
    district = str(parcel.get("district") or "").strip().lower()
    base = next((v for k, v in PLANA_DISTRICT_MODEL.items() if k in district), {"sale": 2800.0, "rent": 13.5})
    area = _num(parcel.get("parcel_extent_m2")) or 0.0
    area_factor = 1.03 if area and area < 700 else (0.97 if area > 5000 else 1.0)
    sale_mid = base["sale"] * area_factor
    rent_mid = base["rent"] * area_factor
    return {
        "sale_low": sale_mid * 0.85,
        "sale_high": sale_mid * 1.15,
        "rent_low": rent_mid * 0.85,
        "rent_high": rent_mid * 1.15,
        "confidence": "low",
        "basis": "PLANA district-calibrated fallback model",
    }
...
def build_decision_intelligence(
    ...
    *,
    ...
    model_market = _model_market_range(parcel_details)
    market_basis = str(market_analysis.get("sale_range_basis") or "")
    market_modelled = sale_low is None or sale_high is None
    if sale_low is None:
        sale_low = model_market["sale_low"]
    if sale_high is None:
        sale_high = model_market["sale_high"]
    sale_mid = (sale_low + sale_high) / 2.0
    confidence = str(market_analysis.get("confidence") or model_market["confidence"]).lower()
    if market_modelled:
        market_basis = model_market["basis"]
    ...
    # Programme and economics per option. These are feasibility-screening ranges,
    # not a final design, QS estimate, valuation or guaranteed rent/sale result.
    rent_range = market_analysis.get("rent_per_m2_month") or {}
    rent_low = _num(rent_range.get("low"))
    rent_high = _num(rent_range.get("high"))
    rent_modelled = rent_low is None or rent_high is None
    if rent_low is None:
        rent_low = model_market["rent_low"]
    if rent_high is None:
        rent_high = model_market["rent_high"]
    option_economics: list[dict[str, Any]] = []
    space_strategy = proposals.get("space_strategy") or {}
    floors = int(_num(space_strategy.get("floors")) or 0) or None
    balcony_total = _num(space_strategy.get("indicative_covered_balcony_veranda_allowance_m2")) or 0.0
    costed_area = gross * SCREENING_BUILT_AREA_FACTOR if gross else 0.0

    def _metric_range(option: dict[str, Any], label: str) -> tuple[float | None, float | None]:
        item = next((m for m in option.get("metrics") or [] if m.get("label") == label), None)
        if not item:
            return None, None
        nums = [float(x) for x in re.findall(r"\d+(?:\.\d+)?", str(item.get("value") or ""))]
        if not nums:
            return None, None
    ...
    for option in proposals.get("development_options") or []:
        kind = str(option.get("type") or "")
        unit_nums = [float(x) for x in re.findall(r"\d+(?:\.\d+)?", str(option.get("headline") or ""))]
        units_low = min(unit_nums[:2]) if unit_nums else None
        units_high = max(unit_nums[:2]) if unit_nums else units_low
        units_mid = ((units_low + units_high) / 2.0) if units_low is not None and units_high is not None else units_low
        net_low, net_high = _metric_range(option, "Net saleable")
        if kind == "house":
            net_low = net_low or (gross * .86 if gross else None)
            net_high = net_high or (gross * .92 if gross else None)
        elif kind == "mixed_use":
            residential = _option_numeric(option, "Residential test area")
            net_low = residential * .78 if residential else None
            net_high = residential * .86 if residential else None
            if units_mid is None and net_low and net_high:
                units_mid = max(1, round(((net_low + net_high) / 2) / 95))
                units_low = max(1, int(units_mid - 1))
                units_high = int(units_mid + 1)
        avg_unit_low = (net_low / units_high) if net_low and units_high else None
        avg_unit_high = (net_high / units_low) if net_high and units_low else None
        balcony_per_unit = balcony_total / units_mid if balcony_total and units_mid else None
        build_low = costed_area * SCREENING_COST_LOW_EUR_M2 if costed_area else None
        build_high = costed_area * SCREENING_COST_HIGH_EUR_M2 if costed_area else None
        value_factor = 0.96 if kind == "house" else (1.04 if kind == "mixed_use" else 1.0)
        rent_factor = 0.92 if kind == "house" else (1.03 if kind == "mixed_use" else 1.0)
        option_sale_low = sale_low * value_factor
        option_sale_high = sale_high * value_factor
        option_rent_low = rent_low * rent_factor
        option_rent_high = rent_high * rent_factor
        sales_low = net_low * option_sale_low if net_low else None
        sales_high = net_high * option_sale_high if net_high else None
        avg_sale_low = sales_low / units_high if sales_low and units_high else None
        avg_sale_high = sales_high / units_low if sales_high and units_low else None
        monthly_rent_low = net_low * option_rent_low if net_low else None
        monthly_rent_high = net_high * option_rent_high if net_high else None
        annual_rent_low = monthly_rent_low * 12 if monthly_rent_low is not None else None
        annual_rent_high = monthly_rent_high * 12 if monthly_rent_high is not None else None
        build_mid = ((build_low + build_high) / 2) if build_low is not None and build_high is not None else None
        annual_mid = ((annual_rent_low + annual_rent_high) / 2) if annual_rent_low is not None and annual_rent_high is not None else None
        gross_yield_on_build = annual_mid / build_mid * 100 if annual_mid and build_mid else None
        soft_low = build_low * (SCREENING_SOFT_COST_PERCENT / 100.0) if build_low is not None else None
        soft_high = build_high * (SCREENING_SOFT_COST_PERCENT / 100.0) if build_high is not None else None
        sales_cost_low = sales_low * (SCREENING_SALES_COST_PERCENT / 100.0) if sales_low is not None else None
        sales_cost_high = sales_high * (SCREENING_SALES_COST_PERCENT / 100.0) if sales_high is not None else None
        total_dev_low = (build_low + soft_low + sales_cost_low) if None not in (build_low, soft_low, sales_cost_low) else None
        total_dev_high = (build_high + soft_high + sales_cost_high) if None not in (build_high, soft_high, sales_cost_high) else None
        residual_land_low = _screening_residual(net_low, gross, option_sale_low, SCREENING_COST_HIGH_EUR_M2, target_profit_on_cost_percent) if net_low and gross else None
        residual_land_high = _screening_residual(net_high, gross, option_sale_high, SCREENING_COST_LOW_EUR_M2, target_profit_on_cost_percent) if net_high and gross else None
        per_unit_rent_low = monthly_rent_low / units_high if monthly_rent_low is not None and units_high else None
        per_unit_rent_high = monthly_rent_high / units_low if monthly_rent_high is not None and units_low else None
        parking_text = next((str(m.get("value")) for m in option.get("metrics") or [] if m.get("label") == "Parking signal"), None)
        mix = option.get("unit_mix_low") or option.get("unit_mix_high")
        mix_label = None
        if isinstance(mix, dict) and mix:
            mix_label = " · ".join(f"{str(k).replace('_', ' ')}: {v}" for k, v in mix.items() if v)
        option_economics.append({
            "type": kind,
            "label": option.get("label"),
            "status": option.get("status"),
            "recommended_units_low": int(units_low) if units_low is not None else None,
            "recommended_units_high": int(units_high) if units_high is not None else None,
            "recommended_mix": mix_label,
            "average_unit_sqm_low": round(avg_unit_low, 1) if avg_unit_low else None,
            "average_unit_sqm_high": round(avg_unit_high, 1) if avg_unit_high else None,
            "floors": floors,
            "balcony_allowance_total_m2": round(balcony_total, 1) if balcony_total else None,
            "balcony_per_unit_m2": round(balcony_per_unit, 1) if balcony_per_unit else None,
            "costed_built_area_m2": round(costed_area, 1) if costed_area else None,
            "construction_cost_low_eur_m2": SCREENING_COST_LOW_EUR_M2,
            "construction_cost_high_eur_m2": SCREENING_COST_HIGH_EUR_M2,
            "total_build_cost_low_eur": round(build_low) if build_low is not None else None,
            "total_build_cost_high_eur": round(build_high) if build_high is not None else None,
            "sale_price_low_eur_m2": round(option_sale_low),
            "sale_price_high_eur_m2": round(option_sale_high),
            "predicted_completed_value_low_eur": round(sales_low) if sales_low is not None else None,
            "predicted_completed_value_high_eur": round(sales_high) if sales_high is not None else None,
            "estimated_sales_value_low_eur": round(sales_low) if sales_low is not None else None,
            "estimated_sales_value_high_eur": round(sales_high) if sales_high is not None else None,
            "average_sale_price_per_unit_low_eur": round(avg_sale_low) if avg_sale_low is not None else None,
            "average_sale_price_per_unit_high_eur": round(avg_sale_high) if avg_sale_high is not None else None,
            "rent_low_eur_m2_month": round(option_rent_low, 2),
            "rent_high_eur_m2_month": round(option_rent_high, 2),
            "predicted_monthly_rent_per_unit_low_eur": round(per_unit_rent_low) if per_unit_rent_low is not None else None,
            "predicted_monthly_rent_per_unit_high_eur": round(per_unit_rent_high) if per_unit_rent_high is not None else None,
            "estimated_monthly_rent_low_eur": round(monthly_rent_low) if monthly_rent_low is not None else None,
            "estimated_monthly_rent_high_eur": round(monthly_rent_high) if monthly_rent_high is not None else None,
            "estimated_annual_rent_low_eur": round(annual_rent_low) if annual_rent_low is not None else None,
            "estimated_annual_rent_high_eur": round(annual_rent_high) if annual_rent_high is not None else None,
            "gross_yield_on_build_cost_percent": round(gross_yield_on_build, 1) if gross_yield_on_build is not None else None,
            "predicted_total_development_cost_low_eur": round(total_dev_low) if total_dev_low is not None else None,
            "predicted_total_development_cost_high_eur": round(total_dev_high) if total_dev_high is not None else None,
            "predicted_residual_land_value_low_eur": round(residual_land_low) if residual_land_low is not None else None,
            "predicted_residual_land_value_high_eur": round(residual_land_high) if residual_land_high is not None else None,
            "valuation_confidence": confidence if not market_modelled else "low",
            "valuation_basis": market_basis,
            "rent_basis": str(market_analysis.get("rent_range_basis") or model_market["basis"]) if rent_modelled else str(market_analysis.get("rent_range_basis") or "automatic rental observations"),
            "parking_strategy": parking_text or "Test parking within the selected development layout",
        })

    land_ranges = [
        (x.get("predicted_residual_land_value_low_eur"), x.get("predicted_residual_land_value_high_eur"))
        for x in option_economics
        if x.get("predicted_residual_land_value_low_eur") is not None and x.get("predicted_residual_land_value_high_eur") is not None
    ]
    predicted_plot_valuation = None
    if land_ranges:
        lows = [float(x[0]) for x in land_ranges]
        highs = [float(x[1]) for x in land_ranges]
        low = min(lows)
        high = max(highs)
        predicted_plot_valuation = {
            "low_eur": round(low),
            "high_eur": round(high),
            "mid_eur": round((low + high) / 2.0),
            "basis": "PLANA residual land valuation across feasible proposal configurations",
            "confidence": confidence if not market_modelled else "low",
            "modelled_market_fallback": market_modelled,
        }
    ...
    market_signal = {
            ...
            "basis": market_basis,
            "modelled_fallback": market_modelled,
        }
    ...
    return {
        ...
        "development_options": option_economics,
        "predicted_plot_valuation": predicted_plot_valuation,
...
class OpportunityFinderRequest(BaseModel):
    ...
    max_results: int = Field(default=30, ge=3, le=60)
...
async def _finder_candidate(feature: dict[str, Any], payload: OpportunityFinderRequest, semaphore: asyncio.Semaphore) -> dict[str, Any] | None:
    ...
    plot_valuation = decision.get("predicted_plot_valuation") or {}
    return {
        "parcel_id": parcel_id,
        ...
        "predicted_plot_value_low_eur": plot_valuation.get("low_eur"),
        "predicted_plot_value_high_eur": plot_valuation.get("high_eur"),
        "predicted_plot_value_mid_eur": plot_valuation.get("mid_eur"),
        "valuation_confidence": plot_valuation.get("confidence"),
...
def _indexed_candidate(row: dict[str, Any], payload: OpportunityFinderRequest) -> dict[str, Any] | None:
    ...
    confidence_raw = _num(row.get("data_confidence"))
    confidence = confidence_raw if confidence_raw is not None else 0.0
    # Missing confidence is unknown evidence, not bad evidence. The old 0-value
    # penalty flattened preliminary indexed parcels to roughly the same 48/100.
    if confidence_raw is None or confidence_raw <= 0:
        risk_adjusted = score
    else:
        risk_adjusted = round(score * (.88 + .12 * confidence / 100))

    capacity = _num(row.get("floor_capacity_m2"))
    parcel_area = _num(row.get("parcel_area_m2"))
    density_signal = None
    if capacity is not None and parcel_area and parcel_area > 0:
        density_signal = _clamp(45 + min(capacity / parcel_area, 2.0) * 25, 45, 95)
    extras: list[tuple[float, float]] = []
    if gap_pct is not None:
        extras.append((gap_pct, .12))
    if payload.scale != "any":
        extras.append((scale_fit, .08))
    if density_signal is not None:
        extras.append((density_signal, .08))
    if extras:
        extra_weight = sum(weight for _, weight in extras)
        risk_adjusted = round(_clamp(risk_adjusted * (1 - extra_weight) + sum(value * weight for value, weight in extras)))
    final_score = max(0, min(100, risk_adjusted))
    best_use = row.get("best_use") or "Development opportunity"
    district_model = _model_market_range({"parcel": {"district": row.get("district"), "parcel_extent_m2": row.get("parcel_area_m2")}})
    net_capacity = capacity * 0.82 if capacity else None
    predicted_plot_low = _screening_residual(net_capacity, capacity, district_model["sale_low"], SCREENING_COST_HIGH_EUR_M2, 20.0) if net_capacity and capacity else None
    predicted_plot_high = _screening_residual(net_capacity * 1.04, capacity, district_model["sale_high"], SCREENING_COST_LOW_EUR_M2, 20.0) if net_capacity and capacity else None
    if predicted_plot_low is not None and predicted_plot_high is not None and predicted_plot_low > predicted_plot_high:
        predicted_plot_low, predicted_plot_high = predicted_plot_high, predicted_plot_low
    return {
        "parcel_id": row.get("parcel_id"),
        "parcel_number": row.get("parcel_number"),
        "district": row.get("district"),
        ...
        "predicted_plot_value_low_eur": round(predicted_plot_low) if predicted_plot_low is not None else None,
        "predicted_plot_value_high_eur": round(predicted_plot_high) if predicted_plot_high is not None else None,
        "predicted_plot_value_mid_eur": round((predicted_plot_low + predicted_plot_high) / 2.0) if predicted_plot_low is not None and predicted_plot_high is not None else None,
...
async def _find_indexed_opportunities(payload: OpportunityFinderRequest) -> dict[str, Any] | None:
    ...
    # Spread results across the visible area instead of returning one dense local
    # cluster simply because adjacent parcels have near-identical index scores.
    if candidates:
        cols = rows_n = 6
        ...
        for candidate in candidates:
            lon = _num(candidate.get("centroid_lon"))
            lat = _num(candidate.get("centroid_lat"))
            if lon is None or lat is None:
                unlocated.append(candidate)
                continue
            col = min(cols - 1, max(0, int((lon - payload.west) / width * cols)))
            row_n = min(rows_n - 1, max(0, int((lat - payload.south) / height * rows_n)))
            buckets.setdefault((col, row_n), []).append(candidate)
        diversified: list[dict[str, Any]] = []
        depth = 0
        cells = sorted(buckets, key=lambda cell: buckets[cell][0]["score"], reverse=True)
        while cells and len(diversified) < payload.max_results:
            progressed = False
            for cell in cells:
                bucket = buckets[cell]
                if depth < len(bucket):
                    ...
                    progressed = True
                    if len(diversified) >= payload.max_results:
                        break
            if not progressed:
                break
            depth += 1
        candidates = diversified + unlocated
    ...
    if len(candidates) < minimum_index_candidates:
        ...
        for row in rows:
            ...
            if len(candidates) >= payload.max_results:
                break
        # Preserve the spatially diversified order; pending-enrichment rows only
        # fill any remaining result slots.
...
<section id="parkingSection" class="section hidden"><h2>Development options</h2><div id="parkingBody"></div></section>
...
function renderDecision(){
 ...
 const devOpts=d.development_options||[];if(devOpts.length){$("parkingSection").classList.remove("hidden");$("parkingBody").innerHTML=devOpts.map((o,i)=>`<div class="best" style="margin-bottom:10px"><div class="eyebrow">OPTION ${i+1} · ${esc(o.status||"screening")}</div><strong>${esc(o.label||o.type||"Development option")}</strong><div class="facts" style="margin-top:10px"><div class="mini"><div class="k">Recommended programme</div><div class="v">${o.recommended_units_low!=null?`${esc(o.recommended_units_low)}–${esc(o.recommended_units_high)} units`:'Programme test'}</div></div><div class="mini"><div class="k">Average unit size</div><div class="v">${o.average_unit_sqm_low!=null?`${fmt(o.average_unit_sqm_low,0)}–${fmt(o.average_unit_sqm_high,0)} m²`:'—'}</div></div><div class="mini"><div class="k">Floors</div><div class="v">${esc(o.floors??'—')}</div></div><div class="mini"><div class="k">Balconies</div><div class="v">${o.balcony_per_unit_m2!=null?`~${fmt(o.balcony_per_unit_m2,0)} m²/unit`:(o.balcony_allowance_total_m2!=null?`${fmt(o.balcony_allowance_total_m2,0)} m² total`:'—')}</div></div></div>${o.recommended_mix?`<div class="note"><b>Suggested mix:</b> ${esc(o.recommended_mix)}</div>`:''}<div class="facts"><div class="mini"><div class="k">Construction €/m²</div><div class="v">${money(o.construction_cost_low_eur_m2)}–${money(o.construction_cost_high_eur_m2)}</div></div><div class="mini"><div class="k">Costed built area</div><div class="v">${fmt(o.costed_built_area_m2,0)} m²</div></div><div class="mini"><div class="k">Total build cost</div><div class="v">${money(o.total_build_cost_low_eur)}–${money(o.total_build_cost_high_eur)}</div></div><div class="mini"><div class="k">Predicted completed value</div><div class="v">${money(o.predicted_completed_value_low_eur)}–${money(o.predicted_completed_value_high_eur)}</div></div><div class="mini"><div class="k">Average sale / unit</div><div class="v">${o.average_sale_price_per_unit_low_eur!=null?`${money(o.average_sale_price_per_unit_low_eur)}–${money(o.average_sale_price_per_unit_high_eur)}`:'—'}</div></div><div class="mini"><div class="k">Rent · whole scheme / month</div><div class="v">${money(o.estimated_monthly_rent_low_eur)}–${money(o.estimated_monthly_rent_high_eur)}</div></div><div class="mini"><div class="k">Rent / unit / month</div><div class="v">${money(o.predicted_monthly_rent_per_unit_low_eur)}–${money(o.predicted_monthly_rent_per_unit_high_eur)}</div></div><div class="mini"><div class="k">Annual rent</div><div class="v">${o.estimated_annual_rent_low_eur!=null?`${money(o.estimated_annual_rent_low_eur)}–${money(o.estimated_annual_rent_high_eur)}`:'—'}</div></div><div class="mini"><div class="k">Total development cost</div><div class="v">${money(o.predicted_total_development_cost_low_eur)}–${money(o.predicted_total_development_cost_high_eur)}</div></div><div class="mini"><div class="k">Proposal land value</div><div class="v">${money(o.predicted_residual_land_value_low_eur)}–${money(o.predicted_residual_land_value_high_eur)}</div></div><div class="mini"><div class="k">Gross yield on build cost</div><div class="v">${o.gross_yield_on_build_cost_percent!=null?`${esc(o.gross_yield_on_build_cost_percent)}%`:'—'}</div></div></div><div class="note"><b>Valuation:</b> ${esc(o.valuation_confidence||"low")} confidence · ${esc(o.valuation_basis||"PLANA model")}</div><div class="note"><b>Rent basis:</b> ${esc(o.rent_basis||"PLANA model")}</div><div class="note"><b>Parking:</b> ${esc(o.parking_strategy||'Test within layout')}</div></div>`).join("")}
...
function renderOpportunityResults(d){
 const panel=$("opportunityPanel");panel.classList.remove("hidden");panel.innerHTML=`<div class="opp-head"><div><div class="eyebrow">PLANA OPPORTUNITY FINDER</div><h2>${esc(d.results?.length||0)} opportunities surfaced</h2></div><button class="close" id="closeOpp">×</button></div><div class="opp-summary">${esc(d.scanned_parcels)} DLS parcels queued · ${esc(d.completed_parcels??d.candidate_parcels??0)} completed · ${esc(d.standout_count)} stand out.${d.partial_results?' Slow parcels were skipped to return results quickly.':''} ${d.budget_status==='tested_on_open'?'Budget is tested precisely when a parcel is opened.':''}</div>${(d.results||[]).slice(0,50).map((x,i)=>`<div class="opp-card" data-i="${i}"><div class="opp-rank"><div><div class="eyebrow">#${i+1} · ${esc(x.score_label)}</div><h3>Parcel ${esc(x.parcel_number||x.parcel_id)} · ${esc(x.quarter||x.municipality||x.district||"Cyprus")}</h3></div><div class="opp-score">${esc(x.score)}</div></div><p>${esc(x.best_use)}${x.units_low!=null?` · ${esc(x.units_low)}–${esc(x.units_high)} indicative units`:""} · ${fmt(x.floor_capacity_m2,0)} m² floor capacity</p>${x.predicted_plot_value_low_eur!=null?`<div class="opp-why"><b>Predicted plot value ${money(x.predicted_plot_value_low_eur)}–${money(x.predicted_plot_value_high_eur)}</b> · ${esc(x.valuation_confidence||"low")} confidence</div>`:""}${x.underused_percent!=null?`<div class="opp-why">${esc(x.underused_percent)}% preliminary development gap</div>`:""}${(x.why||[]).slice(0,2).map(w=>`<div class="opp-why">✓ ${esc(w)}</div>`).join("")}<div class="opp-why" style="color:var(--muted)">${x.score_evidence_status==="capacity_led"?"Capacity-led score · market checked when opened":"Market-enriched score"}</div></div>`).join("")}`;
...
async function runFinder(){
 ...
 try{const d=await post("/api/find-opportunities",{...finderBrief,west:b.getWest(),south:b.getSouth(),east:b.getEast(),north:b.getNorth(),max_results:50},finderController.signal,20000);$("finder").classList.add("hidden");renderOpportunityResults(d)}
