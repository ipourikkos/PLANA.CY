...
MARKET_PUBLIC_PAGES_PER_SOURCE = max(1, min(20, int(os.getenv("PLANA_MARKET_PUBLIC_PAGES_PER_SOURCE", "4"))))
MARKET_PUBLIC_DETAILS_PER_SOURCE = max(10, min(250, int(os.getenv("PLANA_MARKET_PUBLIC_DETAILS_PER_SOURCE", "80"))))
MARKET_PUBLIC_TIMEOUT_SECONDS = max(10, min(90, int(os.getenv("PLANA_MARKET_PUBLIC_TIMEOUT_SECONDS", "35"))))
...
async def main() -> None:
    ...
    claim_retry_seconds = max(5, int(os.getenv("PLANA_DATA_LEASE_CLAIM_RETRY_SECONDS", "15")))
    while True:
    ...
    log("PLANA DATA WORKER V11.1 2GB CRASH-SAFE + MAX PUBLIC MARKET COLLECTION starting")
    log(
